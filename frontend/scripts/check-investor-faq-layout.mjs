...
assert(css.includes("font-size:clamp(30px,3.6vw,44px)"),'Investor FAQ title must stay responsive and compact')
assert(css.includes('line-height:1.02'),'Investor FAQ title must keep a safe line height')
assert(css.includes('min-height:164px'),'Investor FAQ premium hero must remain compact on desktop')
assert(css.includes('flex:0 0 94px;width:94px;height:94px'),'Investor FAQ guide badge must remain compact')
assert(!css.includes('min-height:220px'),'Investor FAQ must not regress to the previous oversized hero')
