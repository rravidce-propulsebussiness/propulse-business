...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [cycleResult, investmentsResult] = await Promise.all([authRequest('/investments/cycle'), authRequest('/investments')])
        if (cancelled) return
        ...
        if (!cancelled) setError(err?.message || 'Unable to load the current investment cycle')
      } finally {
  ...
  const unmaturedFund = active ? rows.filter(row => normalizeStatus(row?.status) === 'ACTIVE' && row?.matures_at && new Date(row.matures_at).getTime() > Date.now()).reduce((sum, row) => sum + Number(row?.amount || 0), 0) : 0
  ...
  const startHref = '/investment?new=1'
  return <>
    <section className="cycle-dashboard">
      <div className="cycle-dashboard-top">
        <div><span className="cycle-kicker">{active ? 'CURRENT INVESTMENT CYCLE' : 'INVESTMENT WORKSPACE'}</span><h1>{active ? `Cycle #${cycle.id} · ${autoInvest ? 'Auto-Invest ON' : 'Auto-Invest OFF'}` : 'No Active Investment Cycle'}</h1><p>{active ? `${String(cycle.status || '').replaceAll('_', ' ')} · Started ${dateTime(cycle.started_at)} · Maturity ${dateTime(cycle.maturity_at)}` : 'Your previous cycle is closed. Current-cycle balances are ₹0. Previous-cycle investment, advertising, lead sales and withdrawals are available in History.'}</p></div>
        <div className="cycle-dashboard-actions">{active ? <><Link className="dashboard-primary-link" to={startHref} aria-disabled={closing} onClick={e => { if (closing) e.preventDefault() }}>＋ Add Investment</Link>{autoInvest && !closing && <a className="secondary link" href="#exit" onClick={e => { e.preventDefault(); document.querySelector('.legacy-investment .investment-pending-card button')?.click() }}>Request Final Exit</a>}</> : <Link className="dashboard-primary-link" to={startHref}>＋ Start Investment</Link>}<Link className="dashboard-primary-link" to="/investment/payouts">Withdrawals →</Link></div>
      </div>
...
export default function InvestmentWithGeneratedFunds() {
  const [newInvestment, setNewInvestment] = useState(new URLSearchParams(window.location.search).get('new') === '1')
  useEffect(() => {
    const sync = () => setNewInvestment(new URLSearchParams(window.location.search).get('new') === '1')
