        ...
        {visibleLeads.map(lead => {
          ...
          return <article className={`lv2-card ${lead.lead_type || 'basic'} ${exclusive ? 'has-exclusive' : ''}`} key={lead.id}>
            ...
            <div className="lv2-facts">
              ...
              {hasValue(location) && <div><span>⌖</span><b>{location}</b></div>}{hasValue(lead.pincode) && <div><span>#</span><b>PIN {lead.pincode}</b></div>}
            ...
            {logged && !claimed && leadAccess.canClaim && <div className="lv2-exclusive"><div><b>{leadAccess.entitlementSource==='new_business'?'Welcome lead entitlement':leadAccess.entitlementSource==='admin'?'Propulse lead entitlement':'Membership access'}</b><span>{leadAccess.entitlementSource==='new_business'?'Included for your newly verified business':leadAccess.entitlementSource==='admin'?'Granted to your verified business':'Included in your current plan'}{leadAccess.remaining !== undefined ? ` · ${leadAccess.remaining} remaining` : ''}</span></div><button disabled={claiming === lead.id} onClick={() => claim(lead)}>{claiming === lead.id ? 'Claiming…' : 'Claim free →'}</button></div>}
