    ...
    {buyModal && <div className="lv2-overlay" onClick={() => setBuyModal(null)}><div className="lv2-buy-modal" onClick={e => e.stopPropagation()}>
      ...
      <div className="lv2-buy-layout">
        <section className="lv2-buy-main">
          ...
          <div className="lv2-wallet-choice"><label><span className="lv2-wallet-icon">▣</span><span><b>Use wallet balance</b><small>${walletBalance > 0 ? 'Wallet is applied after the coupon discount.' : 'No wallet balance available. You can pay the final amount directly.'}</small></span></label><strong>${walletBalance > 0 ? money(walletBalance) : '₹0'}</strong><input type="checkbox" checked={useWallet} onChange={e => setUseWallet(e.target.checked)} /></div>
          <div className="lv2-share-section"><div className="lv2-share-section-head"><span className="lv2-share-stack">▱</span><div><strong>SHARES</strong><small>Select the number of shares you want to purchase.</small></div></div>
            <div className={"lv2-modal-pricing premium-pricing " + (isPro ? 'pro-only' : '')}><div className="lv2-modal-price-head"><span>PACK</span><span>${isPro ? 'PRO PRICE' : 'NORMAL PRICE'}</span>{!isPro && <span>PRO PRICE</span>}</div>
              {(buyModal.pricing?.shares || []).map(p => { const n = Number(p.shares); const normal = Number(p.normal); const pro = Number(p.pro); const saving = Number.isFinite(normal) && Number.isFinite(pro) && normal > pro ? normal - pro : 0; const normalKey = buyModal.id + '-' + n + '-normal-' + (useWallet ? 'wallet' : 'direct'); const proKey = buyModal.id + '-' + n + '-pro-' + (useWallet ? 'wallet' : 'direct'); return <div className="lv2-modal-price-row" key={n}><div className="lv2-share-badge"><strong>${n}</strong><small>${n === 1 ? 'Single share' : String(n) + ' shares'}</small></div>{!isPro && <button className="lv2-modal-price normal" disabled={buyModalClaimed || buying === normalKey} onClick={() => buy(buyModal, n, 'normal')}>${buying === normalKey ? 'Buying…' : money(normal)}</button>}{isPro && <div className="lv2-pro-member-price"><button className="lv2-modal-price pro selected-pro" disabled={buyModalClaimed || buying === proKey} onClick={() => buy(buyModal, n, 'pro')}>${buying === proKey ? 'Buying…' : money(pro)}</button>{saving > 0 && <small>Pro saved ${money(saving)}</small>}</div>}{!isPro && <button className="lv2-modal-price pro" disabled={buyModalClaimed || buying === proKey} onClick={() => buy(buyModal, n, 'pro')}><span>${buying === proKey ? 'Buying…' : money(pro)}</span>{saving > 0 && <small>Save ${money(saving)}</small>}</button>}</div> })}
            </div>
          </div>
          {!isPro && <div className="lv2-pro-hint"><strong>Pro members save more</strong><span>Compare the Pro price above. Upgrade to Pro to unlock Pro pricing.</span><button onClick={() => { setBuyModal(null); setUpgrade(true) }}>View Pro →</button></div>}{buyModalClaimed && <div className="lv2-modal-owned">This lead is already purchased and available in your account.</div>}
        </section>
        <aside className="lv2-purchase-summary"><div className="lv2-summary-head"><div><strong>Purchase Summary</strong><small>Lead #${buyModal.id}</small></div><span>🛒</span></div><div className="lv2-summary-body"><div className="lv2-summary-row"><span>Customer</span><strong>${buyModal.customer_name || buyModal.service_name || 'Business opportunity'}</strong></div><div className="lv2-summary-row"><span>Location</span><strong>${[buyModal.city_name, buyModal.state_name].filter(hasValue).join(' · ') || 'India'}</strong></div><div className="lv2-summary-row"><span>Category</span><strong>${buyModal.industry_name || buyModal.service_name || 'Lead opportunity'}</strong></div><div className="lv2-summary-divider"/><div className="lv2-summary-row"><span>Available packs</span><strong>${(buyModal.pricing?.shares || []).length || 0}</strong></div><div className="lv2-summary-row"><span>Starting price</span><strong>${(() => { const first = (buyModal.pricing?.shares || [])[0] || {}; const value = isPro ? first.pro : first.normal; return money(value) || '—' })()}</strong></div><div className="lv2-summary-divider"/><div className="lv2-summary-total"><span>Choose a pack below</span><strong>Secure checkout</strong></div><div className="lv2-summary-note">🔒 Your existing wallet, coupon and payment flow remains protected.</div></div></aside>
      </div>
    </div></div>}
