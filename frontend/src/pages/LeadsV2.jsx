...
export default function LeadsV2() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const q = new URLSearchParams({ status: 'available', page: String(page), limit: '20' })
        if (tier !== 'all') q.set('leadType', tier)
        const terms = [category?.replaceAll('-', ' '), search.trim()].filter(Boolean).join(' ')
        if (terms) q.set('search', terms)
        ...
        if (live) { setLeads(Array.isArray(d) ? d : (d.items || [])); setPagination(d.pagination || { page, limit: 20, total: Array.isArray(d) ? d.length : d.items?.length || 0, hasNext: false, hasPrevious: page > 1 }) }
  ...
  const buy = async (lead, shares) => {
    ...
    try {
      ...
      setNotice(`Lead #${lead.id} purchased successfully.`); setExpanded(lead.id)
