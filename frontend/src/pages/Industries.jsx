...
export default function Industries() {
  ...
  async function saveModal(e) {
    ...
    try {
      ...
      if (type === 'subcity') { base = '/subcities'; payload = { cityId: form.parentId, name: form.name.trim(), slug: slugify(form.name), pincode: form.pincode.trim() || null, source: item?.source || 'admin' } }
  ...
  async function syncCityCoverage(city) {
    try { setSyncing(`city:${city.id}`); setError(''); setSuccess(''); const result = await request(`/cities/${city.id}/sync-coverage`, { method: 'POST' }); setSuccess(`${city.name}: ${result.pincodes?.added || 0} pincode updates and ${result.subcities?.added || 0} new sub-city records.`); await loadAll() }
    catch (err) { setError(err.message) } finally { setSyncing('') }
  }

  async function syncAllPincodes() {
    try {
