...
export default function Industries() {
  ...
  async function loadAll() {
    ...
    const endpoints = [
      ['industries', '/industries', setIndustries], ['services', '/services', setServices], ['subservices', '/subservices', setSubservices],
      ['states', '/states', setStates], ['cities', '/cities', setCities], ['subcities', '/subcities', setSubcities],
    ]
    const results = await Promise.all(endpoints.map(async ([label, path, setter]) => {
      try { const value = await request(path); setter(collection(value)); return null }
      catch (err) { return `${label}: ${err.message}` }
    }))
  ...
  const cityMatches = (city, q) => {
    ...
    return name.includes(q) || areas.some(area => String(area.name || '').toLowerCase().includes(q) || String(area.pincode || '').includes(q)) || pins.some(pin => String(pin.pincode || '').includes(q) || String(pin.officeName || '').toLowerCase().includes(q))
  }
    ...
    try {
      ...
      if (tab === 'industries') {
        ...
        for (const row of rows) {
          ...
          if (!exists) { const sc = await request('/subcities', jsonOptions('POST', { cityId: city.id, name: row.subcity_name, slug: slugify(row.subcity_name), pincode: row.pincode || null })); sct.push(sc); added++ }
        }
