...
export default function Industries() {
  ...
  async function syncCityCoverage(city) {
    try { setSyncing(String(city.id)); setError(''); setSuccess(''); const result = await request(`/cities/${city.id}/sync-coverage`, { method: 'POST' }); setSuccess(`${city.name}: ${result.pincodes?.added || 0} pincode updates and ${result.subcities?.added || 0} new sub-city records.`); await loadAll() }
