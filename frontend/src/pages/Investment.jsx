...
export default function Investment() {
  ...
  const selectedLocationRule = useMemo(() => {
    if (!access?.locationLimitsEnabled || !stateId) return null
    const exact = cityId ? locationRules.find((rule) => Number(rule.state_id) === Number(stateId) && Number(rule.city_id) === Number(cityId)) : null
    return exact || locationRules.find((rule) => Number(rule.state_id) === Number(stateId) && rule.city_id == null) || null
  }, [access?.locationLimitsEnabled, stateId, cityId, locationRules])
  const stateHasCityRules = useMemo(() => locationRules.some((rule) => Number(rule.state_id) === Number(stateId) && rule.city_id != null), [locationRules, stateId])
  const allowedStateIds = useMemo(() => new Set(locationRules.map((rule) => Number(rule.state_id))), [locationRules])

  const invest = async () => {
    setBusy(true); setMessage('')
    try {
      if (access?.locationLimitsEnabled && !stateId) throw new Error('Select an investment location.')
      if (access?.locationLimitsEnabled && stateHasCityRules && !cityId && !locationRules.some((rule) => Number(rule.state_id) === Number(stateId) && rule.city_id == null)) throw new Error('Select a city for this location.')
  ...
  return (
    <main className="investment-page"><div className="investment-shell">
      ...
      <section className="investment-grid">
        <div className="investment-card"><div className="card-heading"><span className="investment-kicker">START A CYCLE</span><h2>Make an investment</h2><p>Choose an eligible industry, location, and fund it from your Propulse wallet.</p></div><label>Industry<select value={industryId} onChange={(event) => { setIndustryId(event.target.value); setAmount('') }}><option value="">Select industry</option>{rules.map((rule) => <option key={rule.id} value={rule.industry_id}>{rule.industry_name}</option>)}</select></label>{access.locationLimitsEnabled && <><label>State<select value={stateId} onChange={(event) => { setStateId(event.target.value); setCityId(''); setAmount('') }}><option value="">Select state</option>{states.filter((state) => allowedStateIds.has(Number(state.id))).map((state) => <option key={state.id} value={state.id}>{state.name}</option>)}</select></label><label>City<select value={cityId} disabled={!stateId} onChange={(event) => { setCityId(event.target.value); setAmount('') }}><option value="">{stateHasCityRules ? 'Select city / state-wide rule' : 'All cities'}</option>{cityOptions.map((city) => <option key={city.id} value={city.id}>{city.name}</option>)}</select></label>{selectedLocationRule && <div className="investment-range"><span>Location capacity <b>{money(selectedLocationRule.investor_limit)}</b></span><span>{selectedLocationRule.city_name ? `${selectedLocationRule.city_name}, ${selectedLocationRule.state_name}` : `${selectedState?.name || selectedLocationRule.state_name} · all cities`}</span></div>}</>}{selected && <div className="investment-range"><span>Industry minimum <b>{money(selected.minimum_amount)}</b></span><span>Industry maximum <b>{money(selected.maximum_amount)}</b></span></div>}<label>Investment amount<input type="number" min={selected?.minimum_amount || 1} max={Math.min(Number(selected?.maximum_amount || Number.MAX_SAFE_INTEGER), Number(selectedLocationRule?.investor_limit || Number.MAX_SAFE_INTEGER))} value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Enter amount" /></label><button className="investment-submit" disabled={busy || !industryId || !amount || (access.locationLimitsEnabled && !stateId)} onClick={invest}>{busy ? 'Processing…' : 'Invest from Wallet →'}</button><div className="investor-note"><b>What happens at maturity?</b><span>Propulse settles the revenue actually allocated from paid lead sales into your wallet. You then choose: keep the payout or reinvest it.</span></div></div>
        <div className="investment-card"><div className="card-heading"><span className="investment-kicker">ELIGIBLE INDUSTRIES & LOCATIONS</span><h2>Where you can invest</h2><p>Each industry has its own investment range. Location limits can further cap the amount invested in a state or specific city.</p></div><div className="investment-rules">{rules.map((rule) => <div className="investment-rule" key={rule.id}><div><strong>{rule.industry_name}</strong><span>{money(rule.minimum_amount)} – {money(rule.maximum_amount)}</span></div><small>{Number(rule.investor_revenue_share_percent ?? 100)}% of realized eligible lead-sale revenue</small></div>)}</div>{access.locationLimitsEnabled && <div className="investment-rules" style={{ marginTop: 12 }}>{locationRules.map((rule) => <div className="investment-rule" key={rule.id}><div><strong>{rule.city_name || rule.state_name}</strong><span>{money(rule.investor_limit)} capacity</span></div><small>{rule.city_name ? `${rule.city_name}, ${rule.state_name}` : `${rule.state_name} · all cities`}</small></div>)}</div>}</div>
      </section>
