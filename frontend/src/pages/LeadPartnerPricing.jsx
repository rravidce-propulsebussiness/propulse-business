      ...
      <main className="partner-main">
        ...
        <div className="partner-content">
          <section className="partner-intro">
            <div><span className="partner-eyebrow">LEAD PARTNER PORTAL · PRICING</span><h1>Pricing & Revenue</h1><p>Configure Pro pricing for your uploaded leads using the same sharing-tier approach as Admin Lead Pricing.</p></div>
          ...
          <section className="partner-pricing-rule-banner">
            <article><span>ADMIN COMMISSION</span><strong>{settings.commissionPercent ?? 5}%</strong><small>Current partner transaction commission.</small></article>
            <article><span>ADMIN UPLIFT</span><strong>+{money(uplift)}</strong><small>Added automatically when calculating Normal customer price.</small></article>
            <article><span>PARTNER CONTROL</span><strong>PRO ONLY</strong><small>You configure Pro price; Normal stays system-calculated.</small></article>
          </section>

          <section className="partner-panel partner-pricing-manager">
            <div className="partner-panel-head">
              <div><span className="partner-kicker">PRICE CONFIGURATION</span><h2>Your lead pricing matrix</h2><p>Each uploaded lead starts with the Admin Pro pricing. Update only the Pro amounts for its existing share tiers.</p></div>
            ...
            {loading ? <div className="partner-empty">Loading pricing configuration…</div> : !leads.length ? <div className="partner-empty">No uploaded leads are available for pricing yet.</div> : (
              <div className="partner-config-list">
                {leads.map(lead => {
                  const tiers = drafts[lead.id] || [];
                  const locked = ['sold', 'closed', 'invalid'].includes(lead.status);
                  return <article className="partner-config-card" key={lead.id}>
                    <div className="partner-config-card-head">
                      <div><span className={`partner-config-type ${lead.leadType}`}>{cap(lead.leadType)}</span><h3>#{lead.id} · {lead.customerName || 'Customer'}</h3><p>{lead.industryName || '—'} · {lead.serviceName || '—'} · {lead.cityName || '—'}</p></div>
                      <div className="partner-config-meta"><span className={`partner-status ${lead.status}`}>{cap(lead.status)}</span><em>{lead.overridden ? 'Custom Pro' : 'Admin default'}</em></div>
                    </div>
                    <div className="partner-config-head"><span>SHARES</span><span>ADMIN PRO</span><span>YOUR PRO PRICE</span></div>
                    <div className="partner-config-rows">
                      {tiers.map(tier => {
                        const adminPro = Number((lead.adminPricing?.shares || []).find(x => Number(x.shares) === Number(tier.shares))?.pro || 0);
                        return <div className="partner-config-row" key={tier.shares}>
                          <div><strong>{tier.shares}</strong><small>Share{tier.shares === 1 ? '' : 's'}</small></div>
                          <div><strong>{money(adminPro)}</strong><small>Starting price</small></div>
                          <label><span>Pro price</span><div className="partner-money-input"><b>₹</b><input disabled={locked} type="number" min="0" step="0.01" value={tier.pro} onChange={e => changeDraft(lead.id, Number(tier.shares), e.target.value)} /></div></label>
                        </div>;
                      })}
                    </div>
                    <div className="partner-config-foot"><span>{locked ? 'Pricing is locked after this lead is sold or closed.' : `Normal price = Pro price + ${money(uplift)}.`}</span><button disabled={locked || savingId === lead.id} type="button" onClick={() => save(lead)}>{savingId === lead.id ? 'Saving…' : locked ? 'Locked' : 'Save Pro pricing'}</button></div>
                  </article>;
                })}
              </div>
            )}
