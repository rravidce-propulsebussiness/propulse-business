...
function LeadDetails({ item, sold }) {
  ...
  const remaining = Math.max(0, capacity - purchased)
  return <article className={`investor-lead-card ${open ? 'is-open' : ''}`}>
    <div className="investor-lead-summary">
      ...
      <div className="investor-lead-summary-side">
        ...
        <small>{sold ? 'All buyer slots filled' : `${remaining} buyer${remaining === 1 ? '' : 's'} still available`}</small>
...
export default function InvestorInvestmentSection({ type }) {
  ...
  const available=leads.filter(item=>buyerCount(item)<buyerCapacity(item))
  const sold=leads.filter(item=>buyerCount(item)>=buyerCapacity(item))
