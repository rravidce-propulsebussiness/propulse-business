...
export default function Membership() {
  ...
  const findPlan = group => group?.plans.find(p => periodLabel(p).toLowerCase().replace(/\s+/g, '') === cycle.replace(/\s+/g, '')) || group?.plans.find(p => Number(p.billing_months || 1) === ({ monthly: 1, quarterly: 3, halfYearly: 6, yearly: 12 }[cycle])) || group?.plans[0]
  ...
  return <div className="membership-page-shell"><UserHeader /><main className="membership-page">
    ...
    {loading ? <div className="membership-state">Loading membership options…</div> : <section className="membership-plans membership-plans-three">{planGroups.map(group => { const plan = selectedByGroup[group.key]; const isCurrent = currentType === group.type; const investorLocked = group.type === 'investor' && !canAccessInvestor; const investorUnlocked = group.type === 'investor' && canAccessInvestor; return <article className={`membership-plan ${group.type}-plan ${investorLocked ? 'investor-locked' : ''}`} key={group.key}>
      ...
      <div className="card-billing"><div><span>CHOOSE BILLING</span><small>Price updates instantly</small></div><div className="membership-cycles">{cycles.map(k => <button type="button" key={k} className={cycle === k ? 'active' : ''} onClick={() => { setCycle(k); setError(''); setSubmitted(false) }}>{cycleLabel(k)}</button>)}</div></div>
      <div className="membership-price">{plan ? money(plan.price) : '—'}<small>{plan ? ` / ${periodLabel(plan).toLowerCase()}` : ''}</small></div>
      {investorLocked && <div className="saving-note">🔒 Unlock with Pro</div>}{investorUnlocked && <div className="saving-note">✓ Included with active Pro access</div>}{!investorLocked && Number(plan?.discount_percent || 0) > 0 && <div className="saving-note">Save {Number(plan.discount_percent)}% on this billing cycle</div>}
    ...
    <section className="membership-value"><div><span className="membership-kicker">WHY MEMBERSHIP</span><h2>Simple plans. Clear access.</h2><p>Everything is organized inside each plan card, so you can compare pricing, switch billing and choose your membership without jumping around the page.</p></div><div className="value-grid"><div><strong>01</strong><b>Pick a plan</b><span>Compare Pro, Investor and Booster side by side.</span></div><div><strong>02</strong><b>Choose billing</b><span>Monthly, Quarterly, Half-Yearly or Yearly from the card itself.</span></div><div><strong>03</strong><b>Complete payment</b><span>Choose your plan and submit payment proof for verification.</span></div></div></section>
  </main>
