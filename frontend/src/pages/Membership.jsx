...
const displayName = (type) => ({ pro: 'Pro', booster: 'Booster', investor: 'Investment', investment: 'Investment' }[type] || type)
...
const fallbackBenefits = {
  ...
  investor: ['Actual lead-sale revenue participation', 'No fixed or guaranteed return', 'Investor-controlled choice to reinvest realized proceeds'],
...
export default function Membership() {
  ...
  const openPlan = (plan, type) => {
    if ((type === 'investor' || type === 'booster') && !isProMember) { setError(`${displayName(type)} access is available only after you have an active Pro membership.`); return }
    if (!plan?.id) { setError(`The ${displayName(type)} membership plan is unavailable.`); return }
        ...
        {loading ? <div className="membership-state">Loading membership options…</div> : (
          <section className="membership-plans membership-plans-three">
            {groups.map((group) => {
              ...
              const isCurrent = group.type === 'pro' ? currentType === 'pro' : group.type === 'booster' ? Boolean(currentMembership?.isBoosterActive) : Boolean(currentMembership?.isInvestorActive)
              ...
              return (
                <article className={`membership-plan ${group.type}-plan ${locked ? 'access-locked' : ''}`} key={group.key}>
                  <div className="membership-plan-top"><div><span className="plan-label">{group.name.toUpperCase()}</span><h2>{group.name}</h2><p>{locked ? 'Available after activating an active Pro membership.' : group.type === 'investor' ? 'Investment membership for eligible lead-generation opportunities. Results are based on actual realized lead-sale revenue.' : plan?.description || `${group.name} membership for businesses using the Propulse marketplace.`}</p></div>{isCurrent ? <span className="current-badge">CURRENT</span> : group.type === 'pro' ? <span className="popular-badge">RECOMMENDED</span> : locked ? <span className="current-badge">PRO REQUIRED</span> : <span className="popular-badge">UNLOCKED FOR PRO</span>}</div>
                  {group.type === 'investor' && <div className="investor-model"><div><b>ACTUAL SALES ONLY</b><span>No fixed or guaranteed return.</span></div><div><b>LOWER SALES = LOWER PAYOUT</b><span>Your result depends on eligible leads actually sold.</span></div><div><b>INVESTOR CHOICE</b><span>After settlement, you decide whether to keep the payout or reinvest it.</span></div></div>}
                  <div className="card-billing"><div><span>{group.type === 'investor' ? 'INVESTMENT MEMBERSHIP CYCLE' : 'CHOOSE BILLING'}</span><small>Cycle length</small></div><div className="membership-cycles">{cycles.map((key) => <button type="button" key={key} className={selectedCycle === key ? 'active' : ''} onClick={() => setSelectedCycles((previous) => ({ ...previous, [group.key]: key }))}>{cycleLabel(key)}</button>)}</div></div>
                  {group.type === 'investor' && <div className="investor-cycle-note">Investment is now treated as a paid membership add-on like Booster. The selected cycle is the membership period and is billed through the same payment verification flow.</div>}
                  <div className="membership-price">{plan ? money(plan.price) : '—'}<small>{plan ? ` / ${period(plan).toLowerCase()}` : ''}</small></div>
                  {locked && <div className="saving-note">🔒 Unlock with Pro</div>}{investorUnlocked && <div className="saving-note">✓ Investment access unlocked with Pro</div>}{boosterUnlocked && <div className="saving-note">✓ Booster unlocked for Pro members</div>}
                  <div className="membership-divider" /><h3>{group.type === 'investor' ? 'Included with Investment' : `Included with ${group.name}`}</h3><ul>{benefits(plan, group.type).map((item, index) => <li key={`${item}-${index}`}><b>✓</b><span>{item}</span></li>)}</ul>
                  {group.type === 'investor' && <div className="investor-flow"><span>1. Activate Investment membership</span><span>2. Fund an eligible industry from your wallet</span><span>3. Lead sales generate realized revenue</span><span>4. Keep or reinvest the realized payout</span></div>}
