...
export default function Membership() {
  ...
  const planGroups = useMemo(() => {
    ...
    plans.forEach(p => {
      let type = typeName(p)
      // Older Admin records may still use "investment" internally.
      if (type === 'investment') type = 'investor'
      if (!['pro', 'booster', 'investor'].includes(type)) return
      if (!map.has(type)) map.set(type, { key: type, type, name: displayType(type), group: groupName(p), plans: [] })
      map.get(type).plans.push(p)
    })
    // Always render the three customer-facing membership cards. If Investor is
    // not returned by the API yet, the card remains visible and locked for
    // non-Pro users instead of silently disappearing.
  ...
  const openPayment = plan => {
    ...
    if (planType === 'investor' || planType === 'investment') {
      if (!canAccessInvestor) setError('Investor access is available only to customers with an active Pro membership.')
      else setError('Investor access is unlocked by an active Pro membership. It is not purchased separately.')
      return
    }
  ...
  const submitManual = async () => {
    ...
    try {
      ...
      await authRequest('/payments/manual', {
        method: 'POST',
        body: JSON.stringify({ amount: Number(selectedPlan.price), membershipPlanId: selectedPlan.id, manualReference: ref, proofUrl, notes: `${selectedPlan.plan_group || displayType(typeName(selectedPlan))}: ${selectedPlan.name || periodLabel(selectedPlan)}` }),
      })
  ...
  return <div className="membership-page-shell"><UserHeader /><main className="membership-page">
    ...
    {loading ? <div className="membership-state">Loading membership options…</div> : <section className="membership-plans membership-plans-three">{planGroups.map(group => { const plan = selectedByGroup[group.key]; const isCurrent = currentType === group.type; const investorLocked = group.type === 'investor' && !canAccessInvestor; const investorUnlocked = group.type === 'investor' && canAccessInvestor; return <article className={`membership-plan ${group.type}-plan ${investorLocked ? 'investor-locked' : ''}`} key={group.key}><div className="membership-plan-top"><div><span className="plan-label">{group.name.toUpperCase()}</span><h2>{group.name}</h2><p>{investorLocked ? 'Investor access is reserved for customers with an active Pro membership.' : plan?.description || `${group.name} membership for businesses using the Propulse marketplace.`}</p></div>{isCurrent ? <span className="current-badge">CURRENT PLAN</span> : group.type === 'pro' ? <span className="popular-badge">RECOMMENDED</span> : investorLocked ? <span className="current-badge">PRO REQUIRED</span> : null}</div><div className="membership-price">{plan ? money(plan.price) : '—'}<small>{plan ? ` / ${periodLabel(plan).toLowerCase()}` : ''}</small></div>{investorLocked && <div className="saving-note">🔒 Unlock with Pro</div>}{investorUnlocked && <div className="saving-note">✓ Included with active Pro access</div>}{!investorLocked && Number(plan?.discount_percent || 0) > 0 && <div className="saving-note">Save {Number(plan.discount_percent)}% on this billing cycle</div>}<div className="membership-divider" /><h3>Included with {group.name}</h3><ul>{features(plan, fallbackBenefits[group.type]).map((x, i) => <li key={`${x}-${i}`}><b>✓</b><span>{x}</span></li>)}</ul>{isCurrent ? <button className="membership-primary current" disabled>✓ Current {group.name} plan</button> : investorLocked ? <button className="membership-primary current" disabled>🔒 Pro membership required</button> : investorUnlocked ? <button className="membership-primary current" disabled>✓ Investor access unlocked</button> : !plan ? <button className="membership-primary current" disabled>Plan being configured</button> : <button className="membership-primary" onClick={() => openPayment(plan)}>Choose {group.name} <span>→</span></button>}{plan?.lead_entitlements?.length > 0 && <small className="membership-secure">Plan benefits and lead entitlements are configured by Propulse.</small>}</article> })}</section>}
