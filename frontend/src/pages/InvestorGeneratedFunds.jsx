...
export default function InvestorGeneratedFunds(){
  ...
  return <section className="investor-generated-funds">
    <div className="investor-generated-head"><div><span className="investor-generated-kicker">INVESTMENT FUNDS</span><h2>Capital deployed in ads & generated earnings</h2><p>Your investment capital stays deployed for advertising. Only revenue generated from paid lead sales is available to transfer.</p></div><button type="button" className="investor-transfer-button" onClick={()=>{setOpen(true);setError('');setMessage('')}} disabled={available<=0}>Transfer funds</button></div>
    ...
    <div className="investor-generated-grid">
      <article><span>AMOUNT IN ADS</span><strong>{money(data?.amount_in_ads)}</strong><small>Investment capital currently deployed into advertising.</small></article>
      <article><span>TOTAL INVESTMENT CAPITAL</span><strong>{money(data?.total_invested)}</strong><small>Non-cancelled investment capital across your cycles.</small></article>
      ...
      <article><span>TRANSFER RESERVED</span><strong>{money(data?.reserved)}</strong><small>Funds held for pending transfer requests.</small></article>
    </div>
