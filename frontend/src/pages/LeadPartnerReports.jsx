...
export default function LeadPartnerReports(){
  ...
  async function load(){try{setLoading(true);setError('');const data=await authRequest('/lead-reports/mine');setReports(Array.isArray(data)?data:[])}catch(e){setError(e.message||'Unable to load reported leads')}finally{setLoading(false)}}
  useEffect(()=>{load()},[]);
  ...
  const counts=useMemo(()=>({all:reports.length,pending:reports.filter(r=>r.status==='pending').length,verified_fake:reports.filter(r=>r.status==='verified_fake').length,verified_genuine:reports.filter(r=>r.status==='verified_genuine').length,rejected:reports.filter(r=>r.status==='rejected').length}),[reports]);
    ...
    <main className="reports-main">
      ...
      <div className="reports-content">
        ...
        <section className="reports-kpi-grid">
          <article><div className="reports-kpi-icon blue">▤</div><div><span>Total Reports</span><strong>{loading?'—':counts.all}</strong><small>All reports submitted by you</small></div></article>
