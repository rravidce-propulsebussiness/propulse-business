...
export default function InvestmentCycleDashboard() {
  ...
  const unmatured = active ? rows.filter(r => st(r?.status) === 'ACTIVE' && r?.matures_at && new Date(r.matures_at).getTime() > Date.now()).reduce((s, r) => s + Number(r?.investor_earnings || 0), 0) : 0
