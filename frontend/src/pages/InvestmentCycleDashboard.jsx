...
export default function InvestmentCycleDashboard() {
  ...
  const [autoInvestChoice, setAutoInvestChoice] = useState(true)
  ...
  const load = async (initial = false) => {
    ...
    try {
      const [cycleResult, investmentResult, rulesResult, accessResult, walletResult, fundsResult, accountResult] = await Promise.all([
        authRequest('/investments/cycle'),
        authRequest('/investments'),
        authRequest('/investments/rules'),
        authRequest('/investments/access'),
        authRequest('/wallet'),
        authRequest('/investments/funds'),
        authRequest('/investor/payout-account')
      ])
      ...
      if (activeCycle && !candidate.auto_invest) setAutoInvestChoice(false)
      if (activeCycle && candidate.auto_invest) setAutoInvestChoice(true)
      setCycle(activeCycle)
      setRows(activeCycle ? investmentRows.filter(r => Number(r?.cycle_id) === Number(activeCycle.id) && INVESTED.has(st(r?.status))) : [])
      setRules(list(rulesResult))
      setAccess(accessResult || null)
      setWallet(walletResult || null)
      setFunds(fundsResult || null)
      setPayoutAccount(accountResult || null)
      setError('')
    } catch (e) {
      if (initial) setError(e?.message || 'Unable to load the current investment cycle')
    } finally {
  ...
  const config = useMemo(() => {
    const active = rules.filter(rule => rule && rule.is_active !== false)
    if (!active.length) return null
    const mins = active.map(rule => Number(rule.minimum_amount)).filter(Number.isFinite)
    const maxs = active.map(rule => Number(rule.maximum_amount)).filter(Number.isFinite)
  ...
  const auto = active ? Boolean(cycle.auto_invest) : Boolean(autoInvestChoice)
  ...
  const unmatured = active ? rows.filter(r => st(r?.status) === 'ACTIVE' && r?.matures_at && new Date(r.matures_at).getTime() > Date.now()).reduce((s, r) => s + Number(r?.amount || 0), 0) : 0
  const linked = active ? Number(cycle.total_leads ?? 0) : 0
  const sold = active ? Number(cycle.sold_leads ?? 0) : 0
  const final = active ? Number(cycle.final_leads ?? 0) : 0
  const pending = active ? Number(cycle.pending_leads ?? 0) : 0
  const paid = active ? Number(cycle.payout_transferred ?? 0) : 0
  const reserved = active ? Number(cycle.payout_reserved ?? 0) : 0
  const closing = active && ['EXIT_REQUESTED', 'WAITING_FOR_LEADS'].includes(st(cycle.status))
  ...
  const openInvest = () => {
    ...
    setActionError('')
    setMessage('')
    setAutoInvestChoice(active ? auto : true)
    setShowInvest(true)
  }
  ...
  const submitWalletInvestment = async () => {
    ...
    try {
      setBusy(true)
      await authRequest('/investments/checkout', { method: 'POST', body: JSON.stringify({ amount: value, useWallet: true, reinvestmentEnabled: active ? auto : autoInvestChoice }) })
      setShowInvest(false)
      setAmount('')
      setMessage(active ? 'Investment added to your current cycle.' : 'Investment cycle started successfully.')
      await load(false)
    } catch (e) {
      setActionError(e?.message || 'Unable to complete investment.')
    } finally {
      setBusy(false)
    }
  }

  const startDirectPayment = async valueFromForm => {
    setActionError('')
    const value = Number(valueFromForm ?? amount)
    if (!Number.isFinite(value) || value <= 0) return setActionError('Enter an investment amount greater than zero.')
    if (active && autoInvestChoice !== auto) return setActionError(`This cycle is ${auto ? 'Auto-Invest ON' : 'Auto-Invest OFF'}. Additional investments must keep the same mode.`)
    try {
      setBusy(true)
      ...
      const draft = await authRequest('/investments/checkout', { method: 'POST', body: JSON.stringify({ amount: value, useWallet: false, reinvestmentEnabled: active ? auto : autoInvestChoice }) })
      setReceiving(details)
      setDirectPayment(draft)
      setShowInvest(false)
      setActionError('')
    } catch (e) {
      setActionError(e?.message || 'Unable to prepare direct payment.')
    } finally {
      setBusy(false)
    }
  }
  ...
  const submitPaymentProof = async () => {
    ...
    try {
      ...
      await authRequest(`/payments/${directPayment.payment.id}/reference`, { method: 'POST', body: JSON.stringify({ manualReference: paymentReference.trim(), proofUrl, notes: 'Propulse managed investment direct payment' }) })
      setDirectPayment(null)
      setReceiving([])
      setPaymentReference('')
      setPaymentProof(null)
      setMessage('Payment submitted for verification. Your investment will activate after payment approval.')
      await load(false)
    } catch (e) {
      setActionError(e?.message || 'Unable to submit payment proof.')
    } finally {
  ...
  const openWithdrawal = () => {
    setWithdrawAmount(transferable > 0 ? transferable.toFixed(2) : '')
    setActionError('')
    setMessage('')
  ...
  const submitWithdrawal = async () => {
    setActionError('')
    if (!payoutAccount) return setActionError('Please add your Bank Account or UPI before requesting a withdrawal.')
    if (closing) return setActionError('Your current investment cycle is closing. Please wait before requesting a withdrawal.')
    if (!Number.isFinite(withdrawValue) || withdrawValue <= 0) return setActionError('Enter a withdrawal amount greater than zero.')
    if (withdrawValue > transferable + 1e-6) return setActionError('Withdrawal amount exceeds your available earnings.')
    try {
      setWithdrawBusy(true)
      await authRequest('/investments/funds/transfer-request', { method: 'POST', body: JSON.stringify({ amount: withdrawValue, withdrawalType: 'PARTIAL' }) })
      setShowWithdraw(false)
      setWithdrawAmount('')
      setMessage('Withdrawal request submitted. The requested amount is reserved while Propulse processes the transfer.')
      await load(false)
    } catch (e) {
      setActionError(e?.message || 'Unable to submit withdrawal request.')
    } finally {
  ...
  return (
    <section className="cycle-dashboard">
      <div className="cycle-dashboard-top">
        ...
        <div className="cycle-dashboard-actions">{active ? <><button type="button" onClick={openInvest} disabled={closing}>＋ Add Investment</button>{auto && !closing && <button type="button" className="secondary" onClick={() => setMessage('Use the Withdrawal action after eligible earnings are generated.')}>Request Final Exit</button>}</> : <button type="button" onClick={openInvest}>＋ Invest</button>}<button type="button" className="withdraw-dashboard-button" onClick={openWithdrawal}>Withdrawals →</button></div>
      </div>
      {message && <div className="investor-section-success" style={{ marginTop: 12 }}>{message}</div>}
      {error && <div className="investor-section-error" style={{ marginTop: 12 }}>{error}</div>}
      {actionError && <div className="investor-section-error" style={{ marginTop: 12 }}>{actionError}</div>}
      <div className="cycle-dashboard-grid">
        <article className="primary"><span>ACTUAL INVESTMENT</span><strong>{money(actual)}</strong><small>{active ? 'Original capital contributed in this cycle.' : 'No active-cycle investment.'}</small></article>
        <article><span>REINVESTMENT</span><strong>{money(reinvest)}</strong><small>{active ? 'Capital reinvested into this current cycle.' : 'No active-cycle reinvestment.'}</small></article>
        <article><span>TOTAL INVESTMENT</span><strong>{money(total)}</strong><small>{active ? 'Actual investment + reinvestment for this cycle.' : 'No active-cycle investment.'}</small></article>
        <article><span>MY EARNINGS</span><strong>{money(earnings)}</strong><small>{active ? 'My realized earnings from paid lead sales in this cycle.' : 'No active-cycle earnings.'}</small></article>
        <article><span>ADS SPENT</span><strong>{money(spent)}</strong><small>{active ? 'Actual advertising spend recorded in this cycle.' : 'No current-cycle ad spend.'}</small></article>
        <article><span>AVAILABLE FOR BANK TRANSFER</span><strong>{money(transferable)}</strong><small>{active ? 'Eligible current-cycle earnings available for withdrawal.' : 'No active-cycle earnings available for transfer.'}</small>{active && transferable > 0 && !closing && <button type="button" className="cycle-withdraw-button" onClick={openWithdrawal}>Withdraw Earnings</button>}</article>
        <article><span>UNMATURED FUND</span><strong>{money(unmatured)}</strong><small>{active ? 'Current-cycle investment capital whose maturity date has not yet been reached.' : 'No active-cycle unmatured funds.'}</small></article>
      </div>
