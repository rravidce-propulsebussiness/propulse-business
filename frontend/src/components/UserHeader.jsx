...
export default function UserHeader() {
  ...
  return <header className="user-header"><Link className="user-header-brand" to="/" onClick={()=>setOpen(false)}><img src="/brand/propulse-logo.png" alt="Propulse Business"/></Link><nav className={`user-header-nav${open?' open':''}`}><Link className={active('/')} to="/" onClick={()=>setOpen(false)}>Home</Link><Link className={active('/my-leads')} to="/my-leads" onClick={()=>setOpen(false)}>My Leads</Link><Link className={active('/wallet')} to="/wallet" onClick={()=>setOpen(false)}>Wallet</Link><Link className={active('/pricing')} to="/pricing" onClick={()=>setOpen(false)}>Pricing</Link><Link className={active('/membership')} to="/membership" onClick={()=>setOpen(false)}>Membership</Link>{isPro&&<Link className={active('/investment')} to="/investment" onClick={()=>setOpen(false)}>Investment</Link>}<Link className={active('/contact?audience=users')} to="/contact?audience=users" onClick={()=>setOpen(false)}>Contact</Link><Link to="/#upcoming-features" onClick={()=>setOpen(false)}>Upcoming Features</Link><button className="user-header-mobile-logout" onClick={logout}>Logout</button></nav><div className="user-header-right"><Link className="user-profile-pill" to="/profile" aria-label="Open business profile"><span className="user-avatar">{avatarLetter}</span><span className="user-profile-name">{displayName}</span></Link><button className="user-logout" onClick={logout}>Logout</button><button className="user-menu-toggle" aria-label="Open navigation" onClick={()=>setOpen(v=>!v)}>☰</button></div></header>
}
