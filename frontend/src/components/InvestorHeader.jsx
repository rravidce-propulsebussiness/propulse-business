...
export default function InvestorHeader(){
 ...
 return <header className="investor-header"><Link className="investor-header-brand" to="/" onClick={()=>setOpen(false)}><img src="/brand/propulse-logo.png" alt="Propulse Business"/></Link>{isPro&&<div className="investor-header-label">INVESTOR</div>}<nav className={`investor-header-nav${open?' open':''}`}>{nav.map(item=><Link key={item.to} className={isActive(item.to)?'active':''} to={item.to} onClick={()=>setOpen(false)}>{item.label}</Link)}<button className="investor-mobile-logout" onClick={logout}>Logout</button></nav><div className="investor-header-right"><Link className="investor-profile" to="/profile" aria-label="Open business profile"><span className="investor-avatar">{avatar}</span><span className="investor-profile-name">{businessName}</span></Link><button className="investor-logout" onClick={logout}>Logout</button><button className="investor-menu" aria-label="Open investor navigation" onClick={()=>setOpen(value=>!value)}>☰</button></div></header>
}
