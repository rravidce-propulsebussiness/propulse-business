...
export default function AdminInvestments() {
  ...
  const showAccount = investor => {
    ...
    sorted.forEach(record => {
      ...
      const paid = record.status === 'paid' ? Number(record.paid_to_investor || 0) : 0
      if (paid > 0) {
  ...
  return <main className="admin-investments-page">
    ...
    {account && <div className="admin-modal-backdrop" onMouseDown={() => setAccount(null)}><div className="admin-modal account-modal" onMouseDown={e => e.stopPropagation()}><button className="modal-close" onClick={() => setAccount(null)}>×</button><div className="admin-settings-kicker">INVESTOR ACCOUNT</div><h2>{account.investor.user_name || 'Investor'}</h2><p>Complete money flow from contributed capital to advertising, lead revenue and transfers.</p><div className="account-summary-grid"><div><span>Capital contributed</span><strong>{money(account.investor.contributed)}</strong></div><div><span>Total ad funding</span><strong>{money(account.investor.totalFunding)}</strong></div><div><span>Ads spent</span><strong>{money(account.investor.adSpent)}</strong></div><div><span>Current ad balance</span><strong>{money(account.investor.adBalance)}</strong></div><div><span>Lead revenue</span><strong className="positive">{money(account.investor.gross)}</strong></div><div><span>Ready to transfer</span><strong className="transfer-value">{money(account.investor.payable)}</strong></div></div><div className="money-flow-head"><h3>Money Flow</h3><span>Every funding and spend movement is shown here</span></div>{!account.flow.length ? <div className="admin-empty">No money movements recorded yet.</div> : <div className="money-flow">{account.flow.map((item, index) => <div className={`money-flow-row ${item.type}`} key={`${item.date}-${item.type}-${index}`}><div className="flow-icon">{item.type === 'investment' ? '+' : item.type === 'reinvest' ? '↻' : item.type === 'spend' ? '−' : item.type === 'revenue' ? '₹' : '↗'}</div><div className="flow-main"><strong>{item.title}</strong><small>{date(item.date)} · {item.note}</small></div><strong className="flow-amount">{item.amount >= 0 ? '+' : ''}{money(item.amount)}</strong><div className="flow-balance"><span>{item.balanceLabel}</span><strong>{money(item.balance)}</strong></div></div>)}</div>}<div className="account-footer-note"><span>Investor capital is separate from earnings. Advertising balance tracks invested and reinvested funding; earnings balance tracks lead revenue and actual transfers.</span></div></div></div>}
