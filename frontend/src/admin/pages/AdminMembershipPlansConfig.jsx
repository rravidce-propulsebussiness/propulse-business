...
export default function AdminMembershipPlansConfig() {
  ...
  async function req(path, options = {}) { return apiRequest(path, options); }
  async function load() {
    setLoading(true);
    try {
      const [membershipPlans, investorSettings, stateData, cityData] = await Promise.all([
        req('/membership-plans'), req('/admin/commercial/investor-settings'), req('/states'), req('/cities'),
      ]);
      setPlans(membershipPlans); setInvestor(investorSettings);
      setStates(Array.isArray(stateData) ? stateData : []); setCities(Array.isArray(cityData) ? cityData : []);
      setError('');
  ...
  const groups = useMemo(() => {
    const map = {};
    plans.forEach(p => { if (!['pro', 'non_pro', 'booster'].includes(p.plan_type)) return; const key = p.plan_group || p.name.replace(/\s+[^\s]+$/i, ''); (map[`${p.plan_type}:${key}`] ||= []).push(p); });
  ...
  const addonPriceFor = (item, cycle) => { const cfg = item.cycles?.[cycle.key] || {}; if (cycle.key === 'monthly') return { final: Number(cfg.price || 0), base: Number(cfg.price || 0), saving: 0 }; const monthly = Number(item.cycles?.monthly?.price || 0); const base = monthly * cycle.months; const final = base * (1 - Number(cfg.discount || 0) / 100); return { final, base, saving: Math.max(0, base - final) }; };
  function switchPlanTab(next) { setTab(next); setEditing(null); setForm(freshForm(next)); setError(''); setSaveStatus(''); }

  async function create(e) {
    e.preventDefault(); if (saving) return; setError(''); setSaveStatus(''); setSaving(true);
    try {
      if (form.planType === 'booster') {
        const activePeriods = form.periods.filter(p => p.enabled !== false && Number(p.months) > 0); if (!activePeriods.length) throw new Error('Enable at least one Booster billing cycle.');
        ...
        const activePeriods = form.periods.filter(p => p.enabled !== false && Number(p.months) > 0); if (!activePeriods.length) throw new Error('Enable at least one billing cycle.');
      ...
      setSaveStatus('Saved successfully'); setForm(freshForm(form.planType)); await load();
    } catch (e) { setError(e.message); setSaveStatus('Save failed'); } finally { setSaving(false); }
  }

  function beginEdit(plan) {
    setEditing(plan.id); setTab(plan.plan_type === 'booster' ? 'booster' : 'pro'); setSaveStatus('');
    if (plan.plan_type === 'booster') {
      const rawAddons = Array.isArray(plan.add_ons) ? plan.add_ons : []; const addons = rawAddons.length ? rawAddons.map(normalizeAddon) : DEFAULT_BOOSTER_ADDONS.map(normalizeAddon); const periods = BOOSTER_CYCLES.map(c => ({ ...c, enabled: true }));
      setForm({ name: 'Booster', planType: 'booster', monthlyBasePrice: Number(plan.monthly_base_price || 0) || '', periods, pricing: Object.fromEntries(BOOSTER_CYCLES.map(c => [c.key, { discount: 0, price: '', customPrice: false }])), benefits: Array.isArray(plan.benefits) ? plan.benefits : [], addOns: addons });
    } else {
      ...
      setForm({ name: plan.plan_group || plan.name.replace(/\s+[^\s]+$/i, ''), planType: 'pro', monthlyBasePrice: plan.monthly_base_price || '', periods: [{ key, label: plan.billing_period || 'Monthly', months, enabled: true, leadEntitlements: leads.map(x => ({ ...x, monthly_quantity: Number(x.monthly_quantity ?? x.quantity ?? 0), period_total_quantity: Number(x.period_total_quantity ?? (Number(x.monthly_quantity ?? x.quantity ?? 0) * months)) })) }], pricing: { [key]: { discount: Number(plan.discount_percent || 0), price: plan.price ?? '', customPrice: true } }, benefits: Array.isArray(plan.benefits) ? plan.benefits : [], addOns: Array.isArray(plan.add_ons) ? plan.add_ons.map(normalizeAddon) : [] });
    }
  ...
  async function saveEdit(e) {
    ...
    try {
      if (form.planType === 'booster') {
        const activePeriods = form.periods.filter(p => p.enabled !== false && Number(p.months) > 0); if (!activePeriods.length) throw new Error('Enable at least one Booster billing cycle.'); const periods = activePeriods.map(p => ({ ...p, months: Number(p.months) }));
        for (const period of periods) { const cfg = form.pricing[period.key] || {}; const existing = plans.find(p => p.plan_type === 'booster' && p.plan_group === 'Booster' && Number(p.billing_months) === Number(period.months)); const payload = { name: `Booster ${period.label}`, planGroup: 'Booster', planType: 'booster', billingPeriod: period.label, billingMonths: Number(period.months), monthlyBasePrice: Number(form.monthlyBasePrice || 0), discountPercent: Number(cfg.discount || 0), priceOverride: cfg.customPrice && cfg.price !== '' ? Number(cfg.price) : '', benefits: form.benefits, leadEntitlements: [], addOns: form.addOns, leadRolloverEnabled: false, leadExpiryDays: null }; if (existing) await req(`/membership-plans/${existing.id}`, { method: 'PUT', body: JSON.stringify(payload) }); else await req('/membership-plans', { method: 'POST', body: JSON.stringify(payload) }); }
      } else {
        const p = form.periods[0]; const cfg = form.pricing[p.key] || {}; await req(`/membership-plans/${editing}`, { method: 'PUT', body: JSON.stringify({ name: `${form.name.trim()} ${p.label}`, planGroup: form.name.trim(), planType: 'pro', billingPeriod: p.label, billingMonths: Number(p.months), monthlyBasePrice: Number(form.monthlyBasePrice || 0), discountPercent: Number(cfg.discount || 0), priceOverride: cfg.customPrice ? Number(cfg.price || 0) : '', benefits: form.benefits, leadEntitlements: p.leadEntitlements, addOns: form.addOns, leadRolloverEnabled: true, leadExpiryDays: null }) });
      }
      setEditing(null); setForm(freshForm(form.planType)); setSaveStatus('Saved successfully'); await load();
    } catch (e) { setError(e.message); setSaveStatus('Save failed'); } finally { setSaving(false); }
  }
  async function toggle(plan) { try { setError(''); await req(`/membership-plans/${plan.id}/status`, { method: 'PATCH', body: JSON.stringify({ isActive: !plan.is_active }) }); setSaveStatus('Saved successfully'); await load(); } catch (e) { setError(e.message); setSaveStatus('Save failed'); } }
  async function remove(plan) { if (!confirm(`Delete ${plan.name}?`)) return; try { setError(''); await req(`/membership-plans/${plan.id}`, { method: 'DELETE' }); setSaveStatus('Saved successfully'); await load(); } catch (e) { setError(e.message); setSaveStatus('Save failed'); } }
  ...
  async function saveInvestor(e) {
    ...
    try {
      ...
      setSaveStatus('Saved successfully'); await load();
    } catch (e) { setError(e.message); setSaveStatus('Save failed'); } finally { setSaving(false); }
  }

  const updateIndustry = (index, field, value) => setInvestor(current => ({ ...current, industryLimits: (current.industryLimits || []).map((item, i) => i === index ? { ...item, [field]: value } : item) }));
  const updateIndustryLocation = (industryIndex, locationIndex, field, value) => setInvestor(current => ({ ...current, industryLimits: (current.industryLimits || []).map((industry, i) => i !== industryIndex ? industry : { ...industry, locations: (industry.locations || []).map((location, j) => j === locationIndex ? { ...location, [field]: value, ...(field === 'state_id' ? { city_id: null } : {}) } : location) }) }));
  const addIndustryLocation = industryIndex => setInvestor(current => ({ ...current, industryLimits: (current.industryLimits || []).map((industry, i) => i !== industryIndex ? industry : { ...industry, locations: [...(industry.locations || []), { id: `new-${Date.now()}-${industryIndex}`, state_id: states[0]?.id || '', city_id: null, investor_limit: 0, is_active: true }] }) }));
  const removeIndustryLocation = (industryIndex, locationIndex) => setInvestor(current => ({ ...current, industryLimits: (current.industryLimits || []).map((industry, i) => i !== industryIndex ? industry : { ...industry, locations: (industry.locations || []).filter((_, j) => j !== locationIndex) }) }));
  ...
  return <main className="commercial-page membership-config-page">
    ...
    {saveStatus && <div className={`save-status ${saveStatus === 'Saved successfully' ? 'success' : 'error'}`} role="status" aria-live="polite">{saveStatus}</div>}
    <nav className="tabs"><button className={tab === 'pro' ? 'selected' : ''} onClick={() => switchPlanTab('pro')}>Pro</button><button className={tab === 'booster' ? 'selected' : ''} onClick={() => switchPlanTab('booster')}>Booster</button><button className={tab === 'investor' ? 'selected' : ''} onClick={() => { setTab('investor'); setEditing(null); setError(''); setSaveStatus(''); }}>Investor</button></nav>
    ...
    {tab !== 'investor' && <>
      <section className="create-card hero-card">
        ...
        <form onSubmit={editing ? saveEdit : create}>
          ...
          {form.planType === 'booster' && <div className="booster-config"><div className="booster-note">Booster add-on pricing: enter the Monthly price. Other periods calculate automatically from Monthly × months, then apply their discount.</div><div className="editor-section"><div className="benefit-head"><b>Add-ons</b><button type="button" className="mini-action" onClick={addAddon}>＋ Add</button></div><div className="booster-addons">{form.addOns.map((item, index) => <div className="booster-addon" key={index}><div className="addon-title"><input value={item.name} onChange={e => setAddon(index, 'name', e.target.value)} /><button type="button" className="remove-lead" onClick={() => removeAddon(index)}>×</button></div><div className="addon-cycles">{BOOSTER_ADDON_CYCLES.map(cycle => { const cfg = item.cycles?.[cycle.key] || { price: 0, enabled: false, discount: 0 }; const price = addonPriceFor(item, cycle); return <div className="addon-cycle" key={cycle.key}><div className="addon-cycle-head"><label className="check-row"><input type="checkbox" checked={cfg.enabled !== false} onChange={e => setAddonCycle(index, cycle.key, 'enabled', e.target.checked)} /> {cycle.label}</label><span>{cycle.months} mo</span></div>{cycle.key === 'monthly' ? <label>Monthly price ₹<input type="number" min="0" step="0.01" value={cfg.price ?? 0} disabled={cfg.enabled === false} onChange={e => updateAddonMonthly(index, e.target.value)} /></label> : <><label>Discount %<input type="number" min="0" max="100" step="0.01" value={cfg.discount || 0} disabled={cfg.enabled === false} onChange={e => setAddonCycle(index, cycle.key, 'discount', Number(e.target.value || 0))} /></label><div className="addon-auto-price"><span>Auto price</span><strong>{money(price.final)}</strong><small>{money(item.cycles?.monthly?.price || 0)} × {cycle.months}{Number(cfg.discount || 0) > 0 ? ` · ${cfg.discount}% off` : ''}</small></div></>}</div>; })}</div></div>)}</div></div>}
          ...
          <div className="form-footer"><button className="primary create-btn" disabled={saving}>{saving ? 'Saving…' : editing ? 'Save changes' : `Create ${form.name}`}</button>{editing && <button type="button" onClick={() => { setEditing(null); setForm(freshForm(tab)); }}>Cancel</button>}</div>
        </form>
      </section>
