...
export default function AdminMembershipPlansConfig() {
  ...
  const [locationLimitDeleteIds, setLocationLimitDeleteIds] = useState([]);
  ...
  const updateLocationLimit = (index, field, value) => setInvestor(current => ({ ...current, locationLimits: (current.locationLimits || []).map((item, i) => i === index ? { ...item, [field]: value, ...(field === 'state_id' ? { city_id: null } : {}) } : item) }));
  const addLocationLimit = () => setInvestor(current => ({ ...current, locationLimits: [...(current.locationLimits || []), { id: `new-${Date.now()}`, state_id: states[0]?.id || '', city_id: null, investor_limit: 0, is_active: true }] }));
  const removeLocationLimit = index => setInvestor(current => { const item = (current.locationLimits || [])[index]; if (item?.id && !String(item.id).startsWith('new-')) setLocationLimitDeleteIds(ids => [...new Set([...ids, Number(item.id)])]); return { ...current, locationLimits: (current.locationLimits || []).filter((_, i) => i !== index) }; });
