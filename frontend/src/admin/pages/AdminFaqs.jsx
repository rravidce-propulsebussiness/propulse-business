...
const empty={category:'general',question:'',answer:'',sort_order:10,is_active:true};
...
export default function AdminFaqs(){
  ...
  async function load(){try{setLoading(true);setError('');const data=await apiRequest('/admin/faqs?audience=lead_partner');setRows(Array.isArray(data)?data:[])}catch(e){setError(e.message||'Unable to load FAQs')}finally{setLoading(false)}}
  useEffect(()=>{load()},[]);
  ...
  return <main className="admin-faq-page">
    <section className="admin-faq-hero"><div><span className="admin-faq-kicker">CONTENT CONTROL</span><h1>Lead Partner FAQs</h1><p>Create, edit, publish or remove the questions shown in the Lead Partner Help Center.</p></div><div className="admin-faq-hero-mark">?</div></section>
    ...
    <section className="admin-faq-layout">
      ...
      <aside className="admin-faq-panel admin-faq-summary"><span className="admin-faq-kicker">HELP CENTER</span><h2>Lead Partner FAQ</h2><p>Manage the knowledge base used across the Lead Partner portal.</p><div className="admin-faq-summary-stats"><div><strong>{rows.length}</strong><span>Total</span></div><div><strong>{rows.filter(r=>r.is_active).length}</strong><span>Published</span></div><div><strong>{rows.filter(r=>!r.is_active).length}</strong><span>Drafts</span></div></div><div className="admin-faq-category-summary">{categories.map(c=><div key={c[0]}><span>{c[1]}</span><b>{rows.filter(r=>r.category===c[0]).length}</b></div>)}</div></aside>
    </section>
