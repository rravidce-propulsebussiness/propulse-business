...
export default function AdminInvestmentAdAllocations() {
  ...
  const saveAllocation = async () => {
    ...
    const id = allocateModal.id
    setBusy(x => ({ ...x, [`allocate-${id}`]: true }))
    setError('')
    setMessage('')
    try {
      await request(`/investments/admin/${id}/ad-amount`, {
        method: 'PUT',
        body: JSON.stringify({ amount })
      })
      closeAllocate()
      setMessage(`${money(amount)} allocated for ads in cycle #${id}.`)
      await load()
    } catch (e) {
      setError(e.message || 'Unable to allocate advertising funds')
    } finally {
      setBusy(x => ({ ...x, [`allocate-${id}`]: false }))
    }
  }
  ...
  const recordSpend = async () => {
    ...
    const id = spendModal.id
    setBusy(x => ({ ...x, [`spend-${id}`]: true }))
    setError('')
    setMessage('')
    try {
      await request(`/investments/admin/${id}/ad-spend`, {
        method: 'POST',
        body: JSON.stringify({ amount, platform, campaign, spendDate: spendDate || undefined, reference, notes })
      })
      setSpendData(await request(`/investments/admin/${id}/ad-spend`))
      setSpendAmount('')
      setPlatform('')
      setCampaign('')
      setSpendDate('')
      setReference('')
      setNotes('')
      setMessage(`${money(amount)} recorded as ad spend for cycle #${id}.`)
      await load()
    } catch (e) {
      setError(e.message || 'Unable to record ad spend')
    } finally {
      setBusy(x => ({ ...x, [`spend-${id}`]: false }))
    }
  }
  ...
  return (
    <main className="admin-ad-allocation-page">
      <header className="admin-ad-page-header"><h1>Ad Allocations</h1></header>
      ...
      <section className="admin-ad-panel">
        {loading ? <div className="admin-ad-empty">Loading…</div> : !items.length ? <div className="admin-ad-empty">No active investment cycles.</div> : (
          <div className="admin-ad-table-wrap">
            <table>
              <thead><tr><th>Investor</th><th>Cycle</th><th>Investment</th><th>Available for Ads</th><th>Current Ad Allocation</th><th>Spent on Ads</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
                {items.map(item => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td><strong>{money(allocated)}</strong>{allocated > 0 && <div className="admin-ad-progress"><span style={{ width: `${Math.min(100, (spent / allocated) * 100)}%` }} /></div>}<small>{allocated > 0 ? `${Math.round(Math.min(100, (spent / allocated) * 100))}% used · ${money(remaining).replace('₹', '')} remaining` : 'No active ad run'}</small></td>
                      <td><strong>{money(spent)}</strong><small>{Number(item.ad_spent || 0) > spent ? `${money(item.ad_spent)} total` : 'Current run'}</small></td>
                      ...
                      <td>
                        <div className="admin-ad-actions">
                          {state === 'ready' && available > 0 && <button type="button" className="admin-ad-primary" disabled={allocating} onClick={() => openAllocate(item)}>{allocating ? 'Allocating…' : 'Allocate'}</button>}
                          {state === 'spent' && available > 0 && <button type="button" className="admin-ad-primary" disabled={allocating} onClick={() => openAllocate(item)}>{allocating ? 'Allocating…' : 'Allocate Next'}</button>}
                          {remaining > 0 && <button type="button" className="admin-ad-secondary" onClick={() => openSpend(item)}>Add Spend</button>}
                          {allocated > 0 && <button type="button" className="admin-ad-link" onClick={() => openSpend(item)}>View</button>}
                        </div>
                      </td>
                    </tr>
                  )
        ...
        <div className="admin-ad-overlay" onMouseDown={e => e.target === e.currentTarget && closeSpend()}>
          <section className="admin-ad-modal admin-ad-spend-modal" onMouseDown={e => e.stopPropagation()}>
            ...
            {spendLoading ? <div className="admin-ad-empty">Loading…</div> : spendData && <>
              <div className="admin-ad-spend-summary"><div><span>Allocation</span><strong>{money(spendData.investment.amount_in_ads)}</strong></div><div><span>Already Spent</span><strong>{money(spendData.investment.current_ad_spent)}</strong></div><div><span>Remaining</span><strong>{money(spendData.investment.ad_remaining)}</strong></div></div>
              ...
              {Number(spendData.investment.ad_remaining) <= 0 && <div className="admin-ad-spent-banner"><strong>This ad run is fully spent.</strong><span>Allocate the next amount from {money(spendData.investment.funds_available_for_ads)} available for ads.</span></div>}
              <div className="admin-ad-modal-buttons"><button type="button" className="admin-ad-secondary" onClick={closeSpend}>Close</button>{Number(spendData.investment.ad_remaining) > 0 && <button type="button" className="admin-ad-primary" disabled={busy[`spend-${spendModal.id}`]} onClick={recordSpend}>{busy[`spend-${spendModal.id}`] ? 'Saving…' : 'Save Spend'}</button>}</div>
