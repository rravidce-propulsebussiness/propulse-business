  ...
  useEffect(() => {
    ...
    const loadStatement = async (userId, cycleId, box, button) => {
      ...
      try {
        ...
        const spendGroups = await Promise.all(investments.map(async investment => {
          try {
            const result = await apiRequest(`/investments/admin/${investment.id}/ad-spend`)
            return { investmentId: Number(investment.id), spends: Array.isArray(result?.spends) ? result.spends : [] }
          } catch { return { investmentId: Number(investment.id), spends: [] }
        }))

        const spendMap = new Map(spendGroups.map(item => [item.investmentId, item.spends]))
        const events = []
        investments.forEach(investment => {
          events.push({ type: 'investment', label: `Investment #${investment.id} added`, amount: Number(investment.amount || 0), date: investment.created_at })
          ;(spendMap.get(Number(investment.id)) || []).forEach(spend => events.push({ type: 'ad', label: `Ads spent · Investment #${investment.id}`, amount: -Math.abs(Number(spend.amount || 0)), date: spend.created_at || spend.spend_date }))
        })
        sales.forEach(sale => events.push({ type: 'revenue', label: `Lead #${sale.lead_id} sold · ${Number(sale.shares || 1)} ${Number(sale.shares || 1) === 1 ? 'single share' : 'shared shares'}`, amount: Number(sale.investor_earnings || 0), date: sale.sold_at, gross: Number(sale.amount || 0) }))
        ...
        const historyRows = events.map(event => {
          ...
          const cls = event.type === 'ad' ? 'debit' : 'credit'
          const prefix = Number(event.amount || 0) >= 0 ? '+' : '−'
          const gross = event.gross != null ? `<small> · Gross ${money(event.gross)}</small>` : ''
          return `<tr><td class="cycle-event-${event.type}">${event.label}${gross}</td><td class="${cls}">${prefix}${money(Math.abs(Number(event.amount || 0)))}</td><td>${money(runningBalance)}</td><td>${dateTime(event.date)}</td></tr>`
        }).join('')
