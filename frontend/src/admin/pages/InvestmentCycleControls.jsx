...
export default function InvestmentCycleControls() {
  useEffect(() => {
    ...
    const loadCycleHistory = async (panel, userId, cycleId, historyBox, button) => {
    ...
    const renderPanel = async (modal, force = false) => {
      ...
      try {
        ...
        const cycles = all.filter(cycle => Number(cycle.user_id) === Number(userId)).sort((a, b) => Number(b.id || 0) - Number(a.id || 0))
        const panel = document.createElement('div')
        panel.className = 'investor-cycle-panel'
        panel.innerHTML = `<div class="investor-cycle-head"><strong>Investment Cycle History</strong><span>Each cycle has separate money and transaction history</span></div>${cycles.length ? cycles.map(cycle => { const closed = String(cycle.status || '').toUpperCase() === 'CLOSED'; return `<div class="investor-cycle-row"><div class="investor-cycle-info"><strong>Cycle #${cycle.id} · ${cycle.auto_invest ? 'Auto-Invest' : 'Non-Auto'}</strong><small>${money(cycle.principal)} principal · ${Number(cycle.total_leads || 0)} leads · ${Number(cycle.final_leads || 0)} final · ${Number(cycle.pending_leads || 0)} pending</small></div><span class="investor-cycle-status ${closed ? 'closed' : ''}">${cycle.status || '—'}</span><button class="investor-cycle-history-toggle" data-cycle-id="${cycle.id}">View History</button>${closed ? '' : `<div class="investor-cycle-actions"><button class="close-cycle" data-cycle-id="${cycle.id}">Close Cycle</button><button class="finish-cycle" data-cycle-id="${cycle.id}">Finish</button></div>`}</div><div class="investor-cycle-history" data-history-cycle-id="${cycle.id}" hidden></div>` }).join('') : '<div style="padding:8px 0;color:#8497ae;font-size:9px">No investment cycle found.</div>'}`
        ...
        panel.querySelectorAll('.investor-cycle-history-toggle').forEach(button => button.addEventListener('click', () => { const box = panel.querySelector(`[data-history-cycle-id="${button.dataset.cycleId}"]`); if (box) loadCycleHistory(panel, userId, button.dataset.cycleId, box, button) }))
