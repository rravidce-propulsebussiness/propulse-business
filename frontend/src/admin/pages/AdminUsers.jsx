...
export default function AdminUsers() {
  const [users, setUsers] = useState([]), [plans, setPlans] = useState([]), [catalogs, setCatalogs] = useState(null), [subcitiesByCity, setSubcitiesByCity] = useState({});
  ...
  async function loadPlans() {
    try {
      const data = await authRequest('/membership-plans');
      const activePlans = (data || []).filter(p => String(p.plan_type || '').toLowerCase() === 'pro' && p.is_active);
      setPlans(activePlans);
      setMembershipForm(current => {
        if (current.planId && activePlans.some(p => String(p.id) === String(current.planId))) return current;
        const first = activePlans[0];
        return first ? { ...current, planId: String(first.id) } : current;
      });
      return activePlans;
  ...
  function openUser(u) {
    ...
    const matchingPlan = plans.find(p => String(p.id) === String(u.membership_plan_id));
    setMembershipForm({ planId: matchingPlan ? String(matchingPlan.id) : (plans[0] ? String(plans[0].id) : ''), startsAt: dateInput(u.membership_started_at), expiresAt: dateInput(u.membership_expires_at), days: '30' });
  }
  ...
  async function applyMembership(action) {
    ...
    let body = { action };
    if (action === 'activate') {
      if (!membershipForm.planId) { setError('Select an active Pro plan first.'); return; }
      body.planId = membershipForm.planId; body.startsAt = membershipForm.startsAt; body.expiresAt = membershipForm.expiresAt || undefined;
  ...
  return <section className="admin-users-page">
    <div className="users-heading"><div><h1>Users</h1></div><button className="admin-primary-btn" onClick={() => setShowCreate(true)}>+ Create Admin</button></div>
