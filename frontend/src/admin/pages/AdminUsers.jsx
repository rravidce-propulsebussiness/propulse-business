...
export default function AdminUsers() {
  ...
  const active = users.filter(u => u.is_active).length, businesses = users.filter(u => u.role === 'business').length, admins = users.filter(u => u.role === 'admin').length, investors = users.filter(u => u.role === 'investor').length;
  async function convertToInvestor(u) { if (!window.confirm(`Convert ${u.name || u.email} to an Investor account? This removes general business access.`)) return; try { setError(''); const saved = await authRequest(`/admin/users/${u.id}/convert-investor`, { method: 'POST' }); setUsers(c => c.map(v => v.id === u.id ? { ...v, ...saved } : v)); setSelected(v => v && v.id === u.id ? { ...v, ...saved } : v); } catch (e) { setError(e.message); } }
  ...
  return <section className="admin-users-page">
    ...
    <div className="users-stats"><div><span>Total users</span><strong>{userPagination?.total ?? users.length}</strong></div><div><span>Active</span><strong>{active}</strong></div><div><span>Businesses</span><strong>{businesses}</strong></div><div><span>Admins</span><strong>{admins}</strong></div><div><span>Investors</span><strong>{investors}</strong></div></div>
    <div className="users-panel"><div className="users-toolbar"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search name, email, phone or business..."/><select value={role} onChange={e => setRole(e.target.value)}><option value="all">All account types</option><option value="business">Businesses</option><option value="investor">Investors</option><option value="admin">Admins</option></select><select value={status} onChange={e => setStatus(e.target.value)}><option value="all">All status</option><option value="active">Active</option><option value="inactive">Inactive</option></select></div>{error && <div className="users-error">{error}</div>}<div className="users-table-wrap">{loading ? <div className="empty-users">Loading users...</div> : !users.length ? <div className="empty-users">No users found.</div> : <table><thead><tr><th>ID</th><th>USER</th><th>BUSINESS</th><th>SCOPE</th><th>TYPE</th><th>LEAD PARTNER</th><th>STATUS</th><th>JOINED</th><th className="actions-header">ACTIONS</th></tr></thead><tbody>{users.map(u => <tr key={u.id}><td className="user-id">#{u.id}</td><td><div className="user-cell"><span>{(u.name || '?').charAt(0).toUpperCase()}</span><div><b>{u.name}</b><small>{u.email}</small>{u.phone && <small>{u.phone}</small>}</div></div></td><td>{u.business_name || <span className="muted">—</span>}</td><td>{u.role === 'business' ? <span className="scope-count">{u.service_count || 0} services · {u.location_count || 0} locations</span> : <span className="muted">—</span>}</td><td><em className={`role ${u.role}`}>{u.role === 'business' ? 'Business' : u.role === 'investor' ? 'Investor' : 'Admin'}</em></td><td>{u.lead_partner_status ? <em className={`status ${u.lead_partner_status === 'active' ? 'active' : 'inactive'}`}>{u.lead_partner_status === 'active' ? 'Active' : u.lead_partner_status}</em> : <span className="muted">—</span>}</td><td><em className={`status ${u.is_active ? 'active' : 'inactive'}`}>{u.is_active ? 'Active' : 'Inactive'}</em></td><td>{dateOnly(u.created_at)}</td><td className="actions-cell"><div className="actions"><button onClick={() => openUser(u)}>Manage</button><button onClick={() => toggleStatus(u)}>{u.is_active ? 'Deactivate' : 'Activate'}</button>{u.role === 'business' && <button onClick={() => convertToInvestor(u)}>Convert to Investor</button>}</div></td></tr>)}</tbody></table>}</div></div>
