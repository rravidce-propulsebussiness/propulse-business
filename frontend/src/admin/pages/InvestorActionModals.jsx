...
export default function InvestorActionModals({ spendOpen, transferOpen, availableForAds, transferable, payoutAccount, onCloseSpend, onCloseTransfer, onSpend, onTransfer }) {
  ...
  return <div className="investor-action-overlay" role="presentation">
    {spendOpen && <div className="investor-action-modal" role="dialog" aria-modal="true" aria-labelledby="spend-title">
      <div className="investor-action-header"><div><span className="investor-action-eyebrow">ADVERTISING</span><h3 id="spend-title">Spend on Ads</h3><p>Record the actual amount spent on the investor's advertising allocation.</p></div><button type="button" className="investor-action-close" onClick={onCloseSpend} aria-label="Close">×</button></div>
      <div className="investor-action-balance"><span>Available for Ads</span><strong>{money(availableForAds)}</strong></div>
      <form onSubmit={submitSpend}><div className="investor-action-grid">
      ...
      </div><div className="investor-action-footer"><button type="button" className="secondary" onClick={onCloseSpend}>Cancel</button><button type="submit" className="primary">Confirm Spend</button></div></form>
    </div>}

    {transferOpen && <div className="investor-action-modal transfer" role="dialog" aria-modal="true" aria-labelledby="transfer-title">
      <div className="investor-action-header"><div><span className="investor-action-eyebrow">PAYOUT</span><h3 id="transfer-title">Transfer to Account</h3><p>Review the investor's saved payout destination before making the transfer.</p></div><button type="button" className="investor-action-close" onClick={onCloseTransfer} aria-label="Close">×</button></div>
      ...
      <div className="payout-account-card"><div className="payout-account-top"><span>Saved Payout Account</span><strong>{payoutAccount?.method === 'upi' ? 'UPI' : 'Bank Account'}</strong></div>
        {payoutAccount ? (payoutAccount.method === 'upi' ? <div className="payout-details"><div><small>UPI ID</small><b>{payoutAccount.upi_id || 'Not available'}</b></div><div><small>Status</small><b>{payoutAccount.is_verified ? 'Verified' : 'Pending verification'}</b></div></div> : <div className="payout-details"><div><small>Account Holder</small><b>{payoutAccount.account_holder_name || 'Not available'}</b></div><div><small>Bank</small><b>{payoutAccount.bank_name || 'Not available'}</b></div><div><small>Account Number</small><b>{payoutAccount.account_number_masked || 'Not available'}</b></div><div><small>IFSC</small><b>{payoutAccount.ifsc_code || 'Not available'}</b></div></div>) : <div className="payout-empty">No payout account is available for this investor.</div>}
      </div>
      <form onSubmit={submitTransfer}><div className="investor-action-grid"><label><span>UTR / Transfer Reference</span><input value={reference} onChange={e => setReference(e.target.value)} placeholder="Enter transaction reference" required /></label><label><span>Transfer Proof</span><input value={proofUrl} onChange={e => setProofUrl(e.target.value)} placeholder="Proof URL" required /></label></div><div className="investor-action-footer"><button type="button" className="secondary" onClick={onCloseTransfer}>Cancel</button><button type="submit" className="primary transfer-primary">Confirm Transfer</button></div></form>
    </div>}
  </div>
}
