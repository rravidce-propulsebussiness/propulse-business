...
export default function AdminPayments() {
  ...
  const pendingCount = (items) => items.filter((p) => String(p?.status || '').toLowerCase() === 'pending').length
  ...
  return (
    <section className="admin-payments-page">
      ...
      <div className="payment-panel">
        ...
        {tab === 'approvals' ? (
          <>
            <div className="approval-switcher" role="tablist" aria-label="Approval type"><button type="button" className={approvalType === 'wallet' ? 'active' : ''} onClick={() => { setApprovalType('wallet'); setExpandedApproval({}) }}><span>Wallet</span><b>{tstats.pending ?? 0}</b></button><button type="button" className={approvalType === 'leads' ? 'active' : ''} onClick={() => { setApprovalType('leads'); setExpandedApproval({}) }}><span>Leads</span><b>{leadPending}</b></button><button type="button" className={approvalType === 'membership' ? 'active' : ''} onClick={() => { setApprovalType('membership'); setExpandedApproval({}) }}><span>Membership</span><b>{membershipPending}</b></button><button type="button" className={approvalType === 'investment' ? 'active' : ''} onClick={() => { setApprovalType('investment'); setExpandedApproval({}) }}><span>Investment</span><b>{investmentPending}</b></button></div>
