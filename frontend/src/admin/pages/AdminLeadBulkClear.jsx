    ...
    const setup=()=>{
      ...
      if(!button||!bar.contains(button)){
        ...
        button.addEventListener('click',async()=>{
          ...
          const confirmed=window.confirm(`Permanently delete ${ids.length} ${action}?\n\nThis cannot be undone.`);
          if(!confirmed)return;
          ...
          try{
            ...
            const refresh=[...document.querySelectorAll('.v9-panel-head button')].find(x=>/refresh/i.test(x.textContent));
            if(refresh)refresh.click();
            if(failed.length)window.alert(`${deleted} lead(s) deleted. ${failed.length} failed.\n\n${failed.join('\n')}`);
            else window.alert(`${deleted} lead(s) deleted successfully.`);
          }finally{
