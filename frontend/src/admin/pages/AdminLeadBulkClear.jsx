...
export default function AdminLeadBulkClear(){
 useEffect(()=>{
  let disposed=false,button=null,observer=null,frame=0;
  ...
  setup();
  return()=>{disposed=true;window.removeEventListener('propulse:leads-refresh',onSync);document.removeEventListener('input',onFilterEvent);document.removeEventListener('change',onFilterEvent);document.removeEventListener('click',onFilterEvent);observer?.disconnect();if(frame)window.cancelAnimationFrame(frame);if(button?.parentNode)button.parentNode.removeChild(button)};
