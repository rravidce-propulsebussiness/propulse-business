...
function LoggedInHomeRoute() {
  ...
  if (token && user && user.role === 'lead_partner') {
...
function ProRoute() {
  ...
  useEffect(() => {
    ...
    authRequest('/investments/access')
      .then((access) => {
        if (active) {
          setState(access?.isPro === true ? 'allowed' : 'denied');
        }
      })
      .catch(() => {
        if (active) {
