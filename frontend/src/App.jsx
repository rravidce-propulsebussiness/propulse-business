...
function LoggedInHomeRoute() {
  ...
  const user = getUser();
  if (token && user && user.role === 'admin') return <Navigate to="/admin" replace />;
...
function LoggedOutRoute() {
  const token = getToken();
  const user = getUser();
  if (token && user) return <Navigate to={user.role === 'admin' ? '/admin' : '/leads'} replace />;
  return <Outlet />;
}
...
function InvestorWithHeader() {
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get('new') !== '1') return undefined;
  ...
  return <><InvestorHeader /><InvestmentWithGeneratedFunds /></>;
}

function InvestorSectionWithHeader({ type }) {
  return <><InvestorHeader />{type === 'payouts' ? <InvestorPayoutsWithGeneratedFunds /> : <InvestorInvestmentSection type={type} />}</>;
}
...
function App() {
  return <BrowserRouter><Routes>
    ...
    <Route path="/leads" element={<Leads />} /><Route path="/industries" element={<Industries />} />
    ...
    <Route element={<AdminRoute />}><Route element={<AdminLayout />}>
      <Route path="/admin" element={<AdminDashboard />} /><Route path="/admin/industries" element={<Industries />} />
      <Route path="/admin/businesses" element={<Navigate to="/admin/users" replace />} /><Route path="/admin/users" element={<AdminUsers />} />
      <Route path="/admin/payments" element={<AdminPayments />} /><Route path="/admin/payment-receiving" element={<AdminPaymentDetails />} />
      <Route path="/admin/coupons" element={<AdminCoupons />} /><Route path="/admin/wallet-topups" element={<Navigate to="/admin/payments" replace />} />
