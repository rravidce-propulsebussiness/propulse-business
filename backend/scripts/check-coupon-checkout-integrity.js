...
function makeHarness({activeUses=0}={}){
  ...
  const client={query:async(sql,params=[])=>{
    ...
    if(sql.includes('FROM membership_plans WHERE id=$1'))return{rows:[{id:3,name:'Grow',plan_group:'grow',plan_type:'pro',duration_days:30,price:'1000',is_active:true}]};
    if(sql.includes('FROM memberships m JOIN membership_plans p'))return{rows:[]};
