...
async function main(){
  ...
  const pendingInvestment=await scalar(`SELECT COUNT(*)::int AS count FROM investments i JOIN payments p ON p.id=i.payment_id WHERE i.status='pending' AND p.status IN ('rejected','failed')`);if(pendingInvestment)throw new Error(`${pendingInvestment} rejected/failed investment payments left investments pending`)
  const paidInvestment=await scalar(`SELECT COUNT(*)::int AS count FROM investments i JOIN payments p ON p.id=i.payment_id WHERE p.status='paid' AND i.status='pending'`);if(paidInvestment)throw new Error(`${paidInvestment} paid investment payments left investments pending`)
