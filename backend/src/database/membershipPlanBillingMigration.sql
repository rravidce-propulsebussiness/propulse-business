...
ALTER TABLE membership_plans
  ADD COLUMN IF NOT EXISTS billing_period VARCHAR(20) NOT NULL DEFAULT 'yearly',
  ADD COLUMN IF NOT EXISTS monthly_base_price NUMERIC(12,2),
  ADD COLUMN IF NOT EXISTS discount_percent NUMERIC(5,2) NOT NULL DEFAULT 0;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='membership_plans_billing_period_check') THEN
