...
async function adminUpdateStatus(req,res){try{return res.json(await leadPartnerService.updateStatus(req.params.partnerId,req.body?.status));}catch(e){return sendError(res,e.code==='PARTNER_NOT_FOUND'?404:400,e,'Failed to update Lead Partner status',{code:e.code});}}
async function pricing(req,res){try{return res.json(await pricingService.list(req.user.id,req.query||{}));}catch(e){return res.status(500).json({error:'Failed to load partner pricing'});}}
...
async function reports(req,res){try{return res.json(await leadReportService.getLeadPartnerReports(req.user.id))}catch(e){const s=['PARTNER_NOT_FOUND','PARTNER_NOT_ACTIVE'].includes(e.code)?403:500;return res.status(s).json({error:e.message||'Failed to load Lead Partner reports',code:e.code})}}
async function adminPayouts(req,res){try{return res.json(await payoutService.adminList(req.query||{}));}catch(e){return res.status(500).json({error:'Failed to load Lead Partner payouts'});}}
...
module.exports={dashboard,apply,me,createLead,myLeads,adminPartners,adminPartnerFinancials,adminUpdateStatus,pricing,createPricingRule,savePricingRule,deletePricingRule,updatePricing,payoutAccount,savePayoutAccount,funds,requestWithdrawal,adminPayouts,adminProcessPayout,transactions,reports};
