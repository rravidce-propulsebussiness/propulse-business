...
async function setUserRole(req,res){try{return res.json(await adminService.setUserRole({userId:req.params.id,role:req.body?.role,actingAdminId:req.user.id}));}catch(error){const map={INVALID_ROLE:400,NOT_FOUND:404,SELF_ROLE_CHANGE:400,LAST_ADMIN:409};if(map[error.code])return res.status(map[error.code]).json({error:error.message,code:error.code});console.error('Set user role failed:',error.message);return res.status(500).json({error:'Failed to change account type'});}}
