...
async function submitPaymentReference(req,res){
  ...
  }catch(error){console.error('Submit payment reference failed:',error.message);const status=error.code==='DUPLICATE_REFERENCE'?409:['REFERENCE_REQUIRED','PROOF_REQUIRED','INVALID_PROOF','PROOF_TOO_LARGE','PAYMENT_NOT_PENDING'].includes(error.code)?400:error.code==='NOT_FOUND'||error.code==='DRAFT_NOT_FOUND'?404:error.code==='PAYMENT_PENDING'?409:500;sendError(res,status,error,'Failed to submit payment reference',{code:error.code})}
}
...
async function getPayments(req,res){try{const result=await paymentService.getPayments({status:req.query.status,search:req.query.search,page:req.query.page,limit:req.query.limit});const items=(result.items||[]).filter(payment=>payment.purchase_type!=='wallet_topup'&&!(payment.purchase_type==='lead'&&payment.payment_method==='manual'&&payment.status==='pending'&&!String(payment.manual_reference||'').trim()));const visiblePending=items.filter(payment=>payment.status==='pending').length;const visibleTotal=items.length;res.json({...result,items,total:visibleTotal,pages:visibleTotal?Math.ceil(visibleTotal/Math.max(Number(result.limit)||50,1)):0,stats:{...(result.stats||{}),pending:visiblePending,total:visibleTotal}})}catch(error){console.error('Get payments failed:',error.message);res.status(500).json({error:'Failed to fetch payments'})}}
...
module.exports={checkoutMembership,submitPaymentReference,getCurrentMembership,getUserMembershipPayments,getPayments,getMembershipCustomers,getMembershipCustomerDetails,updatePaymentStatus,updateMembership};
