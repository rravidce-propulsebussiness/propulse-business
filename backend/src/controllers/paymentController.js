...
async function getLeadPayments(req,res){
  try{
    const values=[];
    const where=[`p.purchase_type='lead'`,`(p.status<>'pending' OR (p.payment_method='manual' AND COALESCE(p.external_amount,0)>0 AND (COALESCE(BTRIM(p.manual_reference),'')<>'' OR COALESCE(BTRIM(p.proof_url),'')<>'')))`];
