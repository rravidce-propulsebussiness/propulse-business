...
function money(value, code = 'INVALID_AMOUNT') {
  ...
  if (!Number.isFinite(n) || n <= 0) throw Object.assign(new Error('Amount must be greater than zero'), { code });
  return n;
}
...
async function getWallet(userId) {
  ...
  try {
    ...
    return { id: w.id, balance: Number(w.balance), transactions: tx.rows.map(x => ({ ...x, amount: Number(x.amount), balance_after: Number(x.balance_after) })) };
  } catch (e) {
...
async function getTopups(userId) {
  const r = await pool.query(
    `SELECT id,amount,payment_method,reference,proof_url,status,reviewed_at,created_at,updated_at
     FROM wallet_topups WHERE user_id=$1 ORDER BY created_at DESC,id DESC LIMIT 100`, [userId]
  );
...
async function getAdminTopups({ status = 'all', search = '', page = 1, limit = 50 }) {
  ...
  if (status && status !== 'all') {
    ...
    params.push(status); where.push(`wt.status=$${params.length}`);
  }
  ...
  const safeLimit = Math.min(Math.max(Number(limit) || 50, 1), 100), safePage = Math.max(Number(page) || 1, 1);
  const count = await pool.query(`SELECT COUNT(*)::int AS total FROM wallet_topups wt JOIN users u ON u.id=wt.user_id LEFT JOIN business_profiles bp ON bp.user_id=wt.user_id ${where.length ? `WHERE ${where.join(' AND ')}` : ''}`, params);
  const total = count.rows[0].total, offset = (safePage - 1) * safeLimit, dataParams = [...params, safeLimit, offset];
  const q = `SELECT wt.id,wt.user_id,wt.amount,wt.payment_method,wt.reference,wt.proof_url,wt.status,wt.reviewed_by,wt.reviewed_at,wt.created_at,wt.updated_at,
    ...
    FROM wallet_topups wt JOIN users u ON u.id=wt.user_id LEFT JOIN business_profiles bp ON bp.user_id=wt.user_id LEFT JOIN users ru ON ru.id=wt.reviewed_by
    ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY wt.created_at DESC,wt.id DESC LIMIT $${dataParams.length - 1} OFFSET $${dataParams.length}`;
...
async function approveTopup({ topupId, adminId }) {
  ...
  try {
    ...
    const next = Number(locked.balance) + Number(t.amount);
    await client.query(`UPDATE wallets SET balance=$1,updated_at=CURRENT_TIMESTAMP WHERE id=$2`, [next, locked.id]);
    await client.query(`INSERT INTO wallet_transactions(wallet_id,user_id,type,amount,balance_after,reference_type,reference_id,description)
      VALUES($1,$2,'credit',$3,$4,'wallet_topup',$5,'Wallet top-up approved')`, [locked.id, t.user_id, t.amount, next, t.id]);
    const updated = (await client.query(`UPDATE wallet_topups SET status='approved',reviewed_by=$1,reviewed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=$2 AND status='pending' RETURNING *`, [adminId, topupId])).rows[0];
...
async function rejectTopup({ topupId, adminId }) {
  const r = await pool.query(`UPDATE wallet_topups SET status='rejected',reviewed_by=$1,reviewed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=$2 AND status='pending' RETURNING *`, [adminId, topupId]);
...
async function debitForLead(client, { userId, amount, leadPurchaseId }) {
  const value = money(amount);
  if (!Number.isInteger(Number(leadPurchaseId)) || Number(leadPurchaseId) <= 0) throw Object.assign(new Error('Invalid lead purchase reference'), { code: 'INVALID_REFERENCE' });
  const w = await ensureWallet(client, userId);
  const locked = (await client.query(`SELECT id,balance FROM wallets WHERE id=$1 FOR UPDATE`, [w.id])).rows[0];
  const balance = Number(locked.balance);
  if (!Number.isFinite(balance) || balance < value) throw Object.assign(new Error('Insufficient wallet balance'), { code: 'INSUFFICIENT_BALANCE' });
  const next = balance - value;
  await client.query(`UPDATE wallets SET balance=$1,updated_at=CURRENT_TIMESTAMP WHERE id=$2`, [next, locked.id]);
  await client.query(`INSERT INTO wallet_transactions(wallet_id,user_id,type,amount,balance_after,reference_type,reference_id,description)
    VALUES($1,$2,'debit',$3,$4,'lead_purchase',$5,'Lead purchase')`, [locked.id, userId, value, next, leadPurchaseId]);
  return next;
}
