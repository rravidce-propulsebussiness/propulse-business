const pool=require('../config/database');const accessService=require('./leadAccessStrategyService');const {isProMember}=require('./leadReadService');
...
async function getLeadAccessMap(userId,leadIds){
  ...
  const membership=membershipResult.rows[0]||null;const claims=new Map(claimsResult.rows.map(x=>[Number(x.lead_id),x]));const occupiedByLead=new Map(capacityResult.rows.map(x=>[Number(x.lead_id),Number(x.occupied||0)]));const pro=await isProMember(userId);const out={};
  ...
  for(const lead of leadsResult.rows){
    ...
    const exclusiveAt=new Date(new Date(lead.created_at).getTime()+Number(lead.exclusive_delay_days||0)*86400000);if(lead.is_exclusive&&exclusiveAt>now&&!pro){out[lead.id]={authenticated:true,claimed:false,canClaim:false,reason:'Pro Early Access is still active'};continue;}
...
async function claimLead(userId,leadId){
  ...
  try{
    ...
    const exclusiveAt=new Date(new Date(lead.created_at).getTime()+Number(lead.exclusive_delay_days||0)*86400000);const pro=await isProMember(userId,client);if(lead.is_exclusive&&exclusiveAt>new Date()&&!pro)fail('Pro Early Access is still active','EXCLUSIVE_LOCKED');
