    ...
    try{
      ...
      for(const industry of data.industryLimits){
        ...
        for(const location of locations){
          ...
          const rawLimit=Number(location.limit??location.investor_limit??0);
          const limit=Number.isFinite(rawLimit)?Math.max(0,Math.round(rawLimit)):NaN;
