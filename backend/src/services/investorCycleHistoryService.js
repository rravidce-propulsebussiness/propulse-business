...
async function getCycleHistory(userId,cycleId){
  ...
  const payouts=(await pool.query(`SELECT id,amount,status,created_at,processed_at,transfer_reference FROM investor_payout_requests WHERE user_id=$1 AND cycle_id=$2 ORDER BY created_at ASC,id ASC`,[uid,cid])).rows.map(r=>({...r,id:Number(r.id),amount:Number(r.amount||0),occurred_at:r.processed_at||r.created_at}));
  const events=[];
