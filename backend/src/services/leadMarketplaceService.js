...
async function getMarketplacePage({industryId,serviceId,subserviceId,stateId,cityId,status='available',leadType,search,allIndustries,allLocations,userId,role,page=1,limit=20}){
 ...
 conditions.push(`(SELECT COUNT(DISTINCT buyer.user_id)::int FROM (SELECT lp.user_id FROM lead_purchases lp LEFT JOIN payments pcap ON pcap.id=lp.payment_id WHERE lp.lead_id=l.id AND (lp.status='paid' OR (lp.status='pending_payment' AND pcap.status='pending')) UNION SELECT ec.user_id FROM lead_entitlement_claims ec WHERE ec.lead_id=l.id) buyer) < lead_effective_buyer_capacity(l.access_strategy,l.buyer_capacity,l.release_to_two_after_hours,l.release_to_three_after_hours,l.created_at,l.access_capacity_locked)`);
 if(role!=='admin'&&userId){values.push(userId);const p3=';conditions.push(`NOT EXISTS (SELECT 1 FROM lead_purchases lp WHERE lp.lead_id=l.id AND lp.user_id=${p3} AND lp.status='paid')`);values.push(userId);const pClaim=';conditions.push(`NOT EXISTS (SELECT 1 FROM lead_entitlement_claims ec WHERE ec.lead_id=l.id AND ec.user_id=${pClaim})`);values.push(userId);const p4=';conditions.push(`l.investor_user_id IS DISTINCT FROM ${p4}`)}
...
+values.length;conditions.push(`NOT EXISTS (SELECT 1 FROM lead_purchases lp WHERE lp.lead_id=l.id AND lp.user_id=${p3} AND lp.status='paid')`);values.push(userId);const pClaim=`${values.length}`;conditions.push(`NOT EXISTS (SELECT 1 FROM lead_entitlement_claims ec WHERE ec.lead_id=l.id AND ec.user_id=${pClaim})`);values.push(userId);const p4=`${values.length}`;conditions.push(`l.investor_user_id IS DISTINCT FROM ${p4}`)}
 const where=conditions.length?`WHERE ${conditions.join(' AND ')}`:'';
 const from=`leads l LEFT JOIN industries i ON i.id=l.industry_id LEFT JOIN services s ON s.id=l.service_id LEFT JOIN subservices ss ON ss.id=l.subservice_id LEFT JOIN states st ON st.id=l.state_id LEFT JOIN cities c ON c.id=l.city_id`;
 const count=await pool.query(`SELECT COUNT(DISTINCT l.id)::int AS total FROM ${from} ${where}`,values);const total=Number(count.rows[0]?.total||0);const offset=(safePage-1)*safeLimit;
 const pageValues=[...values,safeLimit,offset];
...
module.exports={getMarketplacePage};
+values.length;conditions.push(`NOT EXISTS (SELECT 1 FROM lead_entitlement_claims ec WHERE ec.lead_id=l.id AND ec.user_id=${pClaim})`);values.push(userId);const p4=`${values.length}`;conditions.push(`l.investor_user_id IS DISTINCT FROM ${p4}`)}
 const where=conditions.length?`WHERE ${conditions.join(' AND ')}`:'';
 const from=`leads l LEFT JOIN industries i ON i.id=l.industry_id LEFT JOIN services s ON s.id=l.service_id LEFT JOIN subservices ss ON ss.id=l.subservice_id LEFT JOIN states st ON st.id=l.state_id LEFT JOIN cities c ON c.id=l.city_id`;
 const count=await pool.query(`SELECT COUNT(DISTINCT l.id)::int AS total FROM ${from} ${where}`,values);const total=Number(count.rows[0]?.total||0);const offset=(safePage-1)*safeLimit;
 const pageValues=[...values,safeLimit,offset];
...
module.exports={getMarketplacePage};
+values.length;conditions.push(`l.investor_user_id IS DISTINCT FROM ${p4}`)}
 const where=conditions.length?`WHERE ${conditions.join(' AND ')}`:'';
 const from=`leads l LEFT JOIN industries i ON i.id=l.industry_id LEFT JOIN services s ON s.id=l.service_id LEFT JOIN subservices ss ON ss.id=l.subservice_id LEFT JOIN states st ON st.id=l.state_id LEFT JOIN cities c ON c.id=l.city_id`;
 const count=await pool.query(`SELECT COUNT(DISTINCT l.id)::int AS total FROM ${from} ${where}`,values);const total=Number(count.rows[0]?.total||0);const offset=(safePage-1)*safeLimit;
 const pageValues=[...values,safeLimit,offset];
