...
async function getMarketplacePage({industryId,serviceId,subserviceId,stateId,cityId,status='available',leadType,search,allIndustries,userId,role,page=1,limit=20}){
 ...
 const explicitAllIndustries=String(allIndustries||'')==='1'||String(allIndustries||'').toLowerCase()==='true';
 ...
 // Related-only by default: match the customer's business industry/services and locations. Explicit filters narrow the same marketplace; they do not add a second/global lead source.
 if(role!=='admin'&&userId&&!explicitAllIndustries&&!industryId&&!serviceId&&!subserviceId&&!stateId&&!cityId&&!type&&!q){
   values.push(userId);const p=`$${values.length}`;
   conditions.push(`EXISTS (
     SELECT 1 FROM business_profiles bp
     WHERE bp.user_id=${p}
       AND (
         l.industry_id IS NULL
         OR l.industry_id=bp.industry_id
         OR EXISTS (SELECT 1 FROM business_profile_services bps WHERE bps.business_profile_id=bp.id AND bps.is_active=TRUE AND bps.industry_id=l.industry_id)
       )
       AND (
         l.service_id IS NULL
         OR EXISTS (SELECT 1 FROM business_profile_services bps2 WHERE bps2.business_profile_id=bp.id AND bps2.is_active=TRUE AND bps2.service_id=l.service_id)
         OR NOT EXISTS (SELECT 1 FROM business_profile_services bps3 WHERE bps3.business_profile_id=bp.id AND bps3.is_active=TRUE)
       )
   )`);
   ...
   conditions.push(`EXISTS (
     ...
     WHERE bp2.user_id=${p2}
       AND (
         l.state_id IS NULL
         OR EXISTS (SELECT 1 FROM business_profile_locations bpl WHERE bpl.business_profile_id=bp2.id AND bpl.is_active=TRUE AND bpl.state_id=l.state_id AND (l.city_id IS NULL OR bpl.city_id=l.city_id))
         OR NOT EXISTS (SELECT 1 FROM business_profile_locations bpl2 WHERE bpl2.business_profile_id=bp2.id AND bpl2.is_active=TRUE)
       )
   )`);
 }
 ...
 const pro=await isProMember(userId);const items=rows.map(row=>({...maskLead(row),is_purchased:false,is_accessible:false,is_pro_member:pro,has_exclusive_option:Boolean(row.is_exclusive),exclusive_available:Boolean(row.is_exclusive)&&(!row.exclusive_available_at||new Date(row.exclusive_available_at)<=new Date()||pro),exclusive_can_buy:Boolean(row.is_exclusive)&&(!row.exclusive_available_at||new Date(row.exclusive_available_at)<=new Date()||pro),exclusive_action:Boolean(row.is_exclusive)?'buy':'buy'}));
