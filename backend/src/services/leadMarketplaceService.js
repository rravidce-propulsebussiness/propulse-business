...
async function getMarketplacePage({industryId,serviceId,subserviceId,stateId,cityId,status='available',leadType,search,allIndustries,userId,role,page=1,limit=20}){
 ...
 // Customer marketplace is related-only by default. Explicit filters narrow this related set; they never broaden it.
 if(role!=='admin'&&userId){
   values.push(userId);const p=`$${values.length}`;
   conditions.push(`EXISTS (SELECT 1 FROM business_profiles bp LEFT JOIN business_profile_services bps ON bps.business_profile_id=bp.id AND bps.is_active=TRUE WHERE bp.user_id=${p} AND (l.industry_id IS NULL OR l.industry_id=bps.industry_id OR l.industry_id=bp.industry_id) AND (l.service_id IS NULL OR bps.service_id=l.service_id OR NOT EXISTS (SELECT 1 FROM business_profile_services bps_any WHERE bps_any.business_profile_id=bp.id AND bps_any.is_active=TRUE)) )`);
   values.push(userId);const p2=`$${values.length}`;
   conditions.push(`EXISTS (SELECT 1 FROM business_profiles bp LEFT JOIN business_profile_locations bpl ON bpl.business_profile_id=bp.id AND bpl.is_active=TRUE WHERE bp.user_id=${p2} AND (l.state_id IS NULL OR l.state_id=bpl.state_id OR NOT EXISTS (SELECT 1 FROM business_profile_locations bpl_any WHERE bpl_any.business_profile_id=bp.id AND bpl_any.is_active=TRUE)) AND (l.city_id IS NULL OR l.city_id=bpl.city_id OR l.state_id IS NULL))`);
   values.push(userId);const p3=`$${values.length}`;
   conditions.push(`NOT EXISTS (SELECT 1 FROM lead_purchases lp WHERE lp.lead_id=l.id AND lp.user_id=${p3} AND lp.status IN ('paid','pending_payment'))`);
 }
