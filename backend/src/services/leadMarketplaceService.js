...
async function getMarketplacePage({industryId,serviceId,subserviceId,stateId,cityId,status='available',leadType,search,allIndustries,userId,role,page=1,limit=20}){
 ...
 // The marketplace is a discovery surface, so the default view must not be narrowed to only the buyer's current business-profile services and locations. Explicit filters/search still narrow results, while the user's own purchased/pending leads remain excluded below.
