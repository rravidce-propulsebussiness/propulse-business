...
async function getAdminReports({status='pending',page=1,limit=50}={}){
  ...
  const where=filtered?'WHERE r.status='+sqlBind(1):'';
  const totalValues=filtered?[normalized]:[];
  const totalSql='SELECT COUNT(*)::int AS count FROM lead_reports r '+where;
  const summarySql="SELECT COUNT(*) FILTER(WHERE r.status='pending')::int AS pending,COUNT(*) FILTER(WHERE r.status='verified_fake')::int AS verified_fake,COUNT(*) FILTER(WHERE r.status='verified_genuine')::int AS verified_genuine,COUNT(*) FILTER(WHERE r.status='rejected')::int AS rejected,COUNT(*)::int AS total,(SELECT COUNT(*)::int FROM lead_reporting_controls c WHERE COALESCE(c.false_report_count,0)>=2) AS repeat_reporters FROM lead_reports r";
  const [totalResult,summaryResult]=await Promise.all([pool.query(totalSql,totalValues),pool.query(summarySql)]);
  const total=Number(totalResult.rows[0]?.count||0);
  const summary=summaryResult.rows[0]||{pending:0,verified_fake:0,verified_genuine:0,rejected:0,total:0,repeat_reporters:0};
  const params=filtered?[normalized,safeLimit,offset]:[safeLimit,offset];
  const limitIndex=filtered?2:1;
  const offsetIndex=filtered?3:2;
  const listSql='SELECT r.*,l.customer_name,l.customer_phone,l.customer_email,l.status AS lead_status,i.name AS industry_name,reporter.name AS reporter_name,reporter.email AS reporter_email,reviewer.name AS reviewer_name,COALESCE(control.can_report_leads,TRUE) AS can_report_leads,COALESCE(control.false_report_count,0)::int AS false_report_count,control.restriction_reason FROM lead_reports r JOIN leads l ON l.id=r.lead_id LEFT JOIN industries i ON i.id=l.industry_id LEFT JOIN users reporter ON reporter.id=r.reporter_user_id LEFT JOIN users reviewer ON reviewer.id=r.reviewed_by LEFT JOIN lead_reporting_controls control ON control.user_id=r.reporter_user_id '+where+' ORDER BY r.created_at DESC,r.id DESC LIMIT '+sqlBind(limitIndex)+' OFFSET '+sqlBind(offsetIndex);
  const result=await pool.query(listSql,params);
  return{data:result.rows,summary,pagination:{page:safePage,limit:safeLimit,total,totalPages:Math.ceil(total/safeLimit)}};
}
