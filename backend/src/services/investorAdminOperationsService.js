...
async function getLinkedLeads({ investorId, investmentId = null }) {
  ...
  return (await pool.query(`
    ...
    LEFT JOIN investments x ON x.user_id=$1 AND x.industry_id=l.industry_id AND x.status IN ('active','matured','paid') AND (x.state_id IS NULL OR l.state_id=x.state_id) AND (x.city_id IS NULL OR l.city_id=x.city_id) ${extra}
