...
async function getPayments({status,search,page=1,limit=50}) {
  ...
  if(search){values.push(`%${String(search).trim()}%`);where.push(`(u.name ILIKE $${values.length} OR u.email ILIKE $${values.length} OR COALESCE(bp.business_name,'') ILIKE $${values.length} OR COALESCE(bp.phone,'') ILIKE $${values.length} OR COALESCE(p.manual_reference,'') ILIKE $${values.length} OR COALESCE(mp.name,'') ILIKE $${values.length})`);}
...
async function getMembershipCustomerDetails(userId) {
  const customerResult=await pool.query(`
    SELECT u.id AS user_id,u.name AS user_name,u.email AS user_email,bp.business_name,bp.phone,
...
async function activateMembership(client,payment) {
  ...
  if(current){membershipId=current.id;if(planType==='pro'&&current.membership_plan_id!==plan.id)note=`Pro membership changed from ${current.plan_name} to ${plan.name} and extended from the existing expiry`;else if(current.membership_plan_id!==plan.id)note=`${planType} membership changed from ${current.plan_name} to ${plan.name} and extended from the existing expiry`;await client.query(`UPDATE memberships SET membership_plan_id=$1,payment_id=$2,expires_at=$3,status='active',updated_at=CURRENT_TIMESTAMP WHERE id=$4`,[plan.id,payment.id,end,current.id]);}
