...
async function createManualPayment({userId,amount,manualReference,proofUrl,notes,membershipPlanId}) {
  ...
  if(planType==='booster' && !(await isProMember(userId))) {
  ...
  const duplicate=await pool.query(`SELECT id,status FROM payments WHERE payment_method='manual' AND LOWER(TRIM(COALESCE(manual_reference,'')))=LOWER(TRIM($1)) LIMIT 1`,[String(manualReference).trim()]);
...
async function updatePaymentStatus(id,status,adminId,notes) {
  ...
  try {
    ...
    if(status==='refunded') throw Object.assign(new Error('A pending manual payment cannot be marked refunded'),{code:'INVALID_PAYMENT_TRANSITION'});
    let membership=null;
