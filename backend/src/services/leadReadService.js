...
const normalizeLeadRow=row=>{
  ...
  return{...row,budget:normalizedBudget,custom_fields:custom,buyer_capacity:normalizeBuyerCapacity({...row,custom_fields:custom}),purchased_buyer_count:Math.max(0,Number(row.purchased_buyer_count)||0),requirement,pricing:normalizeLeadPricing(row.pricing)};
};
...
const maskCustomerName=value=>{const text=String(value??'').trim();if(!text)return null;return text.split(/\s+/).map(part=>part?`${part.slice(0,1)}***`:'').filter(Boolean).join(' ');};
...
const maskLead=row=>{const normalized=normalizeLeadRow(row);const custom=normalized?.custom_fields&&typeof normalized.custom_fields==='object'&&!Array.isArray(normalized.custom_fields)?normalized.custom_fields:{};const publicCustom={};Object.entries(custom).forEach(([key,value])=>{if(isMarketplaceCanonicalField(key)||!String(value??'').trim())return;const masked=maskPublicValue(key,value);if(masked!==null&&masked!==undefined&&String(masked).trim())publicCustom[key]=masked;});const publicLead={...normalized};['created_by','investor_user_id','cycle_id','partner_base_pricing','partner_pricing_overridden','partner_pricing_updated_at','pincode','notes','investor_name','investor_email','lead_partner_id','lead_partner_user_id','lead_partner_status','lead_partner_name','lead_partner_email'].forEach(key=>delete publicLead[key]);return{...publicLead,customer_name:maskCustomerName(normalized.customer_name),customer_phone:normalized.customer_phone?maskContactText(normalized.customer_phone):null,customer_email:normalized.customer_email?maskContactText(normalized.customer_email):null,requirement:normalized.requirement?maskContactText(normalized.requirement):normalized.requirement,custom_fields:publicCustom};};
