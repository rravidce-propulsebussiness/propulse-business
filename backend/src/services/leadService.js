...
const {leadSelect,maskLead,normalizeLeadRow,normalizeLeadType,isProMember}=require('./leadReadService');
...
function mergePricing(base,sheet){
  const baseShares=normalizePricingRows(base?.shares);
  const sheetShares=normalizePricingRows(sheet?.shares);
  if(!sheetShares.length)return{shares:baseShares.length?baseShares:DEFAULT_PRICING.shares};
  if(sheetShares.length>=2)return{shares:sheetShares};
  const only=sheetShares[0];
  const configuredTwo=baseShares.find(x=>x.shares===2)||baseShares[1];
  if(!configuredTwo||configuredTwo.shares===only.shares)return{shares:[only]};
  return{shares:[only,{shares:2,normal:configuredTwo.normal,pro:configuredTwo.pro}]};
}
