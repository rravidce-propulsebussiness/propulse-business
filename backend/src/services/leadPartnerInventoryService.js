...
async function resolveLocationFromPincode(pincode, cat, suppliedState, suppliedCity) {
  ...
  // Admin lead import resolves State/City from the live PIN lookup response.
  // If the local city_pincodes cache is empty, use the same response fields
  // cached by getPincode() (district/city/office) against the active catalog.
  if (!city) {
  ...
  // State is canonical from the pincode. If a CSV State was supplied, it must agree with it.
