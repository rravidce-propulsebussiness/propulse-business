...
async function getCycleStatement(userId, cycleId) {
  ...
  if (!cycle) {
    throw Object.assign(new Error('Investment cycle not found'), { code: 'CYCLE_NOT_FOUND' });
  }
  ...
  const investmentRows = (await pool.query(`
    SELECT
      ...
      i.created_at,
      i.updated_at,
      i.reinvestment_enabled,
      i.parent_investment_id,
      COALESCE((
        SELECT SUM(s.amount)
        FROM investment_ad_spends s
        WHERE s.investment_id=i.id
      ),0)::numeric AS ad_spent,
      COALESCE((
  ...
  const ads = (await pool.query(`
    SELECT
      (SELECT COUNT(*)
       ...
       WHERE i.user_id=$1 AND i.cycle_id=$2 AND i.status<>'cancelled')::int AS transactions,
      COALESCE((SELECT SUM(a.amount)
                FROM investment_ad_allocations a
                JOIN investments i ON i.id=a.investment_id
                WHERE i.user_id=$1 AND i.cycle_id=$2 AND i.status<>'cancelled'),0)::numeric AS allocated,
      COALESCE((SELECT SUM(s.amount)
                FROM investment_ad_spends s
                JOIN investments i ON i.id=s.investment_id
                WHERE i.user_id=$1 AND i.cycle_id=$2 AND i.status<>'cancelled'),0)::numeric AS spent
  `, [uid, cid])).rows[0];
    ...
    SELECT
      ...
      COUNT(DISTINCT l.id) FILTER (
        WHERE EXISTS (
          ...
          WHERE lp.lead_id=l.id AND lp.status='paid'
        )
      )::int AS sold,
      COUNT(DISTINCT lp.id) FILTER (
        WHERE lp.status='paid' AND COALESCE(lp.shares,1)=1
      )::int AS single_share_sales,
      COUNT(DISTINCT lp.id) FILTER (
        WHERE lp.status='paid' AND COALESCE(lp.shares,1)>1
      )::int AS shared_sales,
      COALESCE(SUM(lp.shares) FILTER (WHERE lp.status='paid'),0)::int AS shares_sold,
      COUNT(DISTINCT l.id) FILTER (
        WHERE LOWER(COALESCE(l.status,''))='expired'
      )::int AS expired,
      COUNT(DISTINCT l.id) FILTER (
        WHERE LOWER(COALESCE(l.status,''))='closed'
      )::int AS closed,
      COUNT(DISTINCT l.id) FILTER (
        WHERE LOWER(COALESCE(l.status,''))='admin_closed'
      )::int AS admin_closed,
      COUNT(DISTINCT l.id) FILTER (
        WHERE NOT EXISTS (
          SELECT 1 FROM lead_purchases lp2
          WHERE lp2.lead_id=l.id AND lp2.status='paid'
        )
        AND LOWER(COALESCE(l.status,'')) NOT IN ('expired','closed','admin_closed','sold','consumed')
      )::int AS pending
    FROM leads l
    ...
    SELECT
      ...
      COALESCE((
        ...
        AND ira.investment_id IN (
          SELECT i2.id
          FROM investments i2
          WHERE i2.user_id=$1 AND i2.cycle_id=$2 AND i2.status<>'cancelled'
        )
      ),0)::numeric AS investor_earnings
    FROM leads l
    LEFT JOIN lead_purchases lp ON lp.lead_id=l.id
  ...
  for (const row of withdrawalRows) {
    if (String(row.status || '').toLowerCase() === 'paid') {
      events.push({
        ...
        reference_id: row.id,
        description: 'Withdrawal paid · Manual transfer',
        amount: -Math.abs(row.amount),
        occurred_at: row.processed_at || row.requested_at,
      });
    }
  }
  ...
  return {
    cycle: {
      ...
      maturity_at: cycle.maturity_at,
      exit_requested_at: cycle.exit_requested_at,
    ...
    leads: {
      ...
      single_share_sales: Number(leads.single_share_sales || 0),
      shared_sales: Number(leads.shared_sales || 0),
      shares_sold: Number(leads.shares_sold || 0),
