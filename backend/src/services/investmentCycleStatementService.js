...
async function getCycleStatement(userId, cycleId) {
  ...
  const saleRows = (await pool.query(`
    SELECT lp.id AS purchase_id,lp.lead_id,COALESCE(lp.shares,1)::int AS shares,
           lp.amount::numeric AS amount,lp.created_at AS sold_at,
           COALESCE((
             SELECT SUM(ira.allocated_amount)
             FROM investment_revenue_allocations ira
             JOIN investments i ON i.id=ira.investment_id
             WHERE ira.lead_purchase_id=lp.id AND i.user_id=$1 AND i.cycle_id=$2 AND i.status<>'cancelled'
           ),0)::numeric AS investor_earnings
    FROM lead_purchases lp
    JOIN leads l ON l.id=lp.lead_id
