...
function publicAccount(row){
  if (!row) return null
  if (row.method === 'upi') return { id:row.id, method:'upi', upi_id:row.upi_id, is_verified:row.is_verified, is_active:row.is_active }
  const account = String(row.account_number || '')
  return {
    id:row.id, method:'bank', account_holder_name:row.account_holder_name,
    bank_name:row.bank_name, ifsc_code:row.ifsc_code,
    account_number_masked: account ? `••••${account.slice(-4)}` : null,
    is_verified:row.is_verified, is_active:row.is_active,
  }
}

async function get(userId){
  const result = await pool.query(`SELECT * FROM investor_payout_accounts WHERE user_id=$1 AND is_active=TRUE ORDER BY id DESC LIMIT 1`, [Number(userId)])
...
async function save({userId, method, accountHolderName, accountNumber, ifscCode, bankName, upiId}){
  ...
  try {
    ...
    await client.query(`UPDATE investor_payout_accounts SET is_active=FALSE,updated_at=CURRENT_TIMESTAMP WHERE user_id=$1 AND is_active=TRUE`, [Number(userId)])
    const result = await client.query(`INSERT INTO investor_payout_accounts (user_id,method,account_holder_name,account_number,ifsc_code,bank_name,upi_id,is_verified,is_active) VALUES ($1,$2,$3,$4,$5,$6,$7,FALSE,TRUE) RETURNING *`, [Number(userId),details.method,details.accountHolderName || null,details.accountNumber || null,details.ifscCode || null,details.bankName || null,details.upiId || null])
...
async function getInternal(clientOrPool, userId){
  const result = await clientOrPool.query(`SELECT * FROM investor_payout_accounts WHERE user_id=$1 AND is_active=TRUE LIMIT 1`, [Number(userId)])
