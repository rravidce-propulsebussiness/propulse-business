...
async function getDashboard({ search = '', status = 'all', industryId = '' } = {}) {
  ...
  const rows = (await pool.query(`
    SELECT x.id,x.user_id,u.name AS user_name,u.email AS user_email,x.industry_id,i.name AS industry_name,x.state_id,s.name AS state_name,x.city_id,c.name AS city_name,x.amount,x.status,x.starts_at,x.matures_at,x.maturity_days,x.created_at,x.updated_at,x.reinvestment_enabled,
  ...
  for(const row of rows){
    ...
    const investor=investorsMap.get(row.user_id); investor.investment_count+=1; if(!['cancelled','pending'].includes(String(row.status).toLowerCase()))investor.total_invested+=row.amount; if(row.status==='active')investor.active_invested+=row.amount; if(row.status!=='paid'&&row.status!=='cancelled'&&new Date(row.matures_at)<=new Date())investor.matured_unpaid+=1; investor.linked_leads+=row.linked_leads; investor.sold_linked_leads+=row.sold_linked_leads; investor.allocated_sales+=row.allocated_sales; investor.linked_gross_sales+=row.linked_gross_sales; investor.allocated_revenue+=row.allocated_revenue; investor.paid_to_investor+=row.paid_to_investor; investor.payable_now+=row.payable_now; investor.cycles.push(row);
  }
  const investors=Array.from(investorsMap.values()).map(item=>({...item,total_invested:Number(item.total_invested.toFixed(2)),active_invested:Number(item.active_invested.toFixed(2)),linked_gross_sales:Number(item.linked_gross_sales.toFixed(2)),allocated_revenue:Number(item.allocated_revenue.toFixed(2)),paid_to_investor:Number(item.paid_to_investor.toFixed(2)),payable_now:Number(item.payable_now.toFixed(2))})).sort((a,b)=>b.total_invested-a.total_invested||String(a.user_name||'').localeCompare(String(b.user_name||'')));
  const stats={investors:investors.length,investment_cycles:rows.length,total_invested:Number(rows.filter(x=>!['cancelled','pending'].includes(String(x.status).toLowerCase())).reduce((sum,x)=>sum+x.amount,0).toFixed(2)),active_invested:Number(rows.filter(x=>x.status==='active').reduce((sum,x)=>sum+x.amount,0).toFixed(2)),matured_unpaid:rows.filter(x=>x.status!=='paid'&&x.status!=='cancelled'&&new Date(x.matures_at)<=new Date()).length,linked_leads:Number(investors.reduce((sum,x)=>sum+x.linked_leads,0)),sold_linked_leads:Number(investors.reduce((sum,x)=>sum+x.sold_linked_leads,0)),allocated_sales:Number(investors.reduce((sum,x)=>sum+x.allocated_sales,0)),allocated_revenue:Number(investors.reduce((sum,x)=>sum+x.allocated_revenue,0).toFixed(2)),paid_to_investors:Number(investors.reduce((sum,x)=>sum+x.paid_to_investor,0).toFixed(2)),payable_now:Number(investors.reduce((sum,x)=>sum+x.payable_now,0).toFixed(2))};
