...
async function getDashboard({ search = '', status = 'all', industryId = '' } = {}) {
  ...
  const rows = (await pool.query(`
    SELECT x.id,x.user_id,u.name AS user_name,u.email AS user_email,x.industry_id,i.name AS industry_name,x.state_id,s.name AS state_name,x.city_id,c.name AS city_name,x.amount,x.amount_in_ads,x.ad_spend_status,x.status,x.starts_at,x.matures_at,x.maturity_days,x.created_at,x.updated_at,x.reinvestment_enabled,
      ...
      COALESCE(ad.total_spent,0)::numeric AS ad_spent,COALESCE(ad.current_spent,0)::numeric AS current_ad_spent,COALESCE(ad.current_remaining,0)::numeric AS ad_remaining,COALESCE(ad.funds_available, x.amount)::numeric AS funds_available_for_ads,COALESCE(ad.current_allocation,0)::numeric AS current_ad_allocation,
      COALESCE(ad.total_allocated,0)::numeric AS total_ad_allocated,
    ...
    LEFT JOIN LATERAL (SELECT COALESCE(SUM(ias.amount),0) AS total_spent FROM investment_ad_spends ias WHERE ias.investment_id=x.id) adtotal ON TRUE
    LEFT JOIN LATERAL (
      SELECT aa.amount AS current_allocation,COALESCE(SUM(s2.amount),0) AS current_spent,
        GREATEST(0,aa.amount-COALESCE(SUM(s2.amount),0)) AS current_remaining,
        COALESCE((SELECT SUM(s3.amount) FROM investment_ad_spends s3 WHERE s3.investment_id=x.id),0) AS total_spent,
        COALESCE((SELECT SUM(a3.amount) FROM investment_ad_allocations a3 WHERE a3.investment_id=x.id),0) AS total_allocated
      FROM investment_ad_allocations aa
      LEFT JOIN investment_ad_spends s2 ON s2.allocation_id=aa.id
      WHERE aa.investment_id=x.id AND aa.status IN ('allocated','spending')
      GROUP BY aa.id,aa.amount
  ...
  `, values)).rows.map(row => {
    ...
    const currentAllocation = Number(row.current_allocation || 0);
    const currentSpent = Number(row.current_spent || 0);
    const currentRemaining = Math.max(0, currentAllocation - currentSpent);
    return {
      ...row,
      amount:Number(row.amount||0),amount_in_ads:currentAllocation,ad_spent:totalSpent,current_ad_spent:currentSpent,ad_remaining:currentRemaining,current_ad_allocation:currentAllocation,total_ad_allocated:Number(row.total_allocated||0),
      funds_available_for_ads:Number(Math.max(0,Number(row.amount||0)-totalSpent-currentRemaining).toFixed(2)),
      allocated_revenue:Number(row.allocated_revenue||0),linked_gross_sales:Number(row.linked_gross_sales||0),paid_to_investor:Number(row.paid_to_investor||0),linked_leads:Number(row.linked_leads||0),sold_linked_leads:Number(row.sold_linked_leads||0),linked_paid_sales:Number(row.linked_paid_sales||0),allocated_sales:Number(row.allocated_sales||0),reinvestment_enabled:Boolean(row.reinvestment_enabled),linked_lead_ids:Array.isArray(row.linked_lead_ids)?row.linked_lead_ids.map(Number):[]
    };
  });
  const investorsMap=new Map();
  for(const row of rows){
    if(!investorsMap.has(row.user_id))investorsMap.set(row.user_id,{user_id:row.user_id,user_name:row.user_name,user_email:row.user_email,investment_count:0,total_invested:0,active_invested:0,matured_unpaid:0,linked_leads:0,sold_linked_leads:0,allocated_sales:0,linked_gross_sales:0,allocated_revenue:0,paid_to_investor:0,payable_now:0,ad_allocated:0,ad_spent:0,ad_remaining:0,funds_available_for_ads:0,cycles:[],leadIds:new Set()});
    const investor=investorsMap.get(row.user_id); investor.investment_count+=1; investor.total_invested+=row.amount; if(row.status==='active')investor.active_invested+=row.amount; if(row.status!=='paid'&&new Date(row.matures_at)<=new Date())investor.matured_unpaid+=1;
    investor.ad_allocated+=row.current_ad_allocation; investor.ad_spent+=row.ad_spent; investor.ad_remaining+=row.ad_remaining; investor.funds_available_for_ads+=row.funds_available_for_ads;
    for(const leadId of row.linked_lead_ids) investor.leadIds.add(leadId);
    investor.allocated_sales+=row.allocated_sales; investor.allocated_revenue+=row.allocated_revenue; investor.paid_to_investor+=row.paid_to_investor; investor.payable_now+=0; investor.cycles.push(row);
  }
  const investors=Array.from(investorsMap.values()).map(item=>{const linkedIds=Array.from(item.leadIds);return{...item,linked_leads:linkedIds.length,sold_linked_leads:0,total_invested:Number(item.total_invested.toFixed(2)),active_invested:Number(item.active_invested.toFixed(2)),linked_gross_sales:0,allocated_revenue:Number(item.allocated_revenue.toFixed(2)),paid_to_investor:Number(item.paid_to_investor.toFixed(2)),payable_now:Number(item.payable_now.toFixed(2)),ad_allocated:Number(item.ad_allocated.toFixed(2)),ad_spent:Number(item.ad_spent.toFixed(2)),ad_remaining:Number(item.ad_remaining.toFixed(2)),funds_available_for_ads:Number(item.funds_available_for_ads.toFixed(2)),leadIds:undefined};}).sort((a,b)=>b.total_invested-a.total_invested||String(a.user_name||'').localeCompare(String(b.user_name||'')));
  ...
  const stats={investors:investors.length,investment_cycles:rows.length,total_invested:Number(rows.reduce((sum,x)=>sum+x.amount,0).toFixed(2)),active_invested:Number(rows.filter(x=>x.status==='active').reduce((sum,x)=>sum+x.amount,0).toFixed(2)),matured_unpaid:rows.filter(x=>x.status!=='paid'&&new Date(x.matures_at)<=new Date()).length,linked_leads:Number(investors.reduce((sum,x)=>sum+x.linked_leads,0)),sold_linked_leads:Number(investors.reduce((sum,x)=>sum+x.sold_linked_leads,0)),allocated_sales:Number(investors.reduce((sum,x)=>sum+x.allocated_sales,0)),allocated_revenue:Number(investors.reduce((sum,x)=>sum+x.allocated_revenue,0).toFixed(2)),paid_to_investors:Number(investors.reduce((sum,x)=>sum+x.paid_to_investor,0).toFixed(2)),payable_now:Number(investors.reduce((sum,x)=>sum+x.payable_now,0).toFixed(2)),ad_allocated:Number(investors.reduce((sum,x)=>sum+x.ad_allocated,0).toFixed(2)),ad_spent:Number(investors.reduce((sum,x)=>sum+x.ad_spent,0).toFixed(2)),ad_remaining:Number(investors.reduce((sum,x)=>sum+x.ad_remaining,0).toFixed(2)),funds_available_for_ads:Number(investors.reduce((sum,x)=>sum+x.funds_available_for_ads,0).toFixed(2))};
