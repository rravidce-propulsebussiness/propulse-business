...
async function getDashboard({ search = '', status = 'all', industryId = '' } = {}) {
  ...
  const rows = (await pool.query(`
    SELECT x.id,x.user_id,u.name AS user_name,u.email AS user_email,x.industry_id,i.name AS industry_name,x.state_id,s.name AS state_name,x.city_id,c.name AS city_name,x.amount,x.amount_in_ads,x.ad_spend_status,x.status,x.starts_at,x.matures_at,x.maturity_days,x.created_at,x.updated_at,x.reinvestment_enabled,
      ...
      COALESCE(adtotal.total_spent,0)::numeric AS ad_spent,COALESCE(ad.current_spent,0)::numeric AS current_ad_spent,COALESCE(ad.current_remaining,0)::numeric AS ad_remaining,COALESCE(ad.current_allocation,0)::numeric AS current_ad_allocation,COALESCE(adtotal.total_allocated,0)::numeric AS total_ad_allocated,
  ...
  `, values)).rows.map(row => {
    ...
    return {
      ...row,
      amount:Number(row.amount||0),amount_in_ads:currentAllocation,ad_spent:totalSpent,current_ad_spent:currentSpent,ad_remaining:currentRemaining,current_ad_allocation:currentAllocation,total_ad_allocated:Number(row.total_ad_allocated||0),
  ...
  for(const row of rows){
    ...
    investor.allocated_sales+=row.allocated_sales; investor.allocated_revenue+=row.allocated_revenue; investor.paid_to_investor+=row.paid_to_investor; investor.cycles.push(row);
  }
