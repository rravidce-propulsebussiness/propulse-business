...
async function getHistory(userId) {
  ...
  const wallet = walletResult.rows.map(x => ({
    id:`wallet-${x.id}`,source:'wallet',kind:'wallet',transaction_id:x.id,type:x.type,amount:Number(x.amount),balance_after:Number(x.balance_after),reference_type:x.reference_type,reference_id:x.reference_id,payment_id:x.payment_id,status:x.status,payment_status:x.payment_status,
