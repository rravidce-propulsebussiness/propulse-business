...
async function updateLeadPaymentStatus({paymentId,status,adminId,notes}) {
  ...
  try {
    ...
    if (status === 'paid') {
      if (Number(payment.external_amount) > 0 && !String(payment.manual_reference || '').trim()) {
        throw Object.assign(new Error('Customer payment reference / UTR is required before approval'),{code:'REFERENCE_REQUIRED'});
      }
      if (Number(payment.external_amount) > 0 && !String(payment.proof_url || '').trim()) {
        throw Object.assign(new Error('Customer payment proof is required before approval'),{code:'PROOF_REQUIRED'});
      }
      ...
      if (Number(payment.wallet_amount) > 0 && !payment.wallet_transaction_id) {
        ...
        if (balance + 0.000001 < required) {
          throw Object.assign(
            new Error(`Wallet balance is insufficient to capture ₹${required.toFixed(2)}. Current balance is ₹${balance.toFixed(2)}.`),
            {code:'INSUFFICIENT_BALANCE'}
          );
        }

        const debit = await walletService.debitForPayment(client,{
          userId:payment.user_id,
          amount:required,
          paymentId:payment.id,
          referenceType:'lead',
          referenceId:payment.purchase_id,
          description:`Lead #${payment.purchase_id} purchase`
        });

        if (Number(debit.walletAmount) + 0.000001 < required) {
          throw Object.assign(new Error('Unable to capture the reserved wallet amount for this payment'),{code:'INSUFFICIENT_BALANCE'});
        }
      ...
      const updated = (await client.query(`
        UPDATE payments
        SET status='paid',
            wallet_transaction_id=COALESCE($1,wallet_transaction_id),
            paid_at=CURRENT_TIMESTAMP,
            notes=COALESCE($2,notes),
            updated_at=CURRENT_TIMESTAMP
        WHERE id=$3
        RETURNING *
      `,[payment.wallet_transaction_id,notes || null,payment.id])).rows[0];
    ...
    const updated = (await client.query(`
      ...
      WHERE id=$3
      RETURNING *
    `,[status,notes || null,payment.id])).rows[0];
