...
async function getAvailableForAds(client, userId) {
  const row = (await client.query(`
    SELECT
      ...
      COALESCE((SELECT SUM(a.allocated_amount) FROM investment_revenue_allocations a JOIN investments i ON i.id=a.investment_id WHERE i.user_id=$1 AND i.status <> 'cancelled' AND COALESCE(i.reinvestment_enabled,FALSE)=TRUE),0) AS reinvestable_earnings,
      COALESCE((SELECT SUM(i.payout_amount) FROM investments i WHERE i.user_id=$1 AND i.status='paid'),0) AS paid_earnings
  ...
  return {
    ...
    reinvestable,
    paid,
    available: Math.max(0, capital + reinvestable - spent - paid),
  };
}
...
async function recordSpend({ userId, amount, platform, campaign, spendDate, reference, notes, adminId }) {
  ...
  try {
    ...
    return {
      ...
      investment_id:Number(investment.id),
      allocation_id:Number(allocation.id),
      total_spent:Number(after.spent.toFixed(2)),
      available_for_ads:Number(after.available.toFixed(2)),
      capital:Number(after.capital.toFixed(2)),
      reinvestable_earnings:Number(after.reinvestable.toFixed(2)),
    };
