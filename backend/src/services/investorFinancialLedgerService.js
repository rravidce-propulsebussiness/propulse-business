 ...
 * only to the extent that they have not been consumed by advertising or paid/reserved.
 */
async function getInvestorFinancialSummary(userId, client = pool) {
  ...
  // Reinvestment children are funded by auto-invest earnings, so their actual
  // ad spend is direct evidence that auto-invest earnings were consumed.
  // For spending recorded against original investments, only the portion beyond
  // contributed capital can consume the auto-invest pool.
  ...
  const availableForAds=Math.max(0,capital+autoInvestEarnings-autoDeductions-adSpent);
